...
class PythonTranspilerBackend:
    ...
    def transpile(self, ts_file: Path) -> TranspilationResult:
        ...
        try:
            ts_content = ts_file.read_text(encoding='utf-8', errors='ignore')
            
            # Generate header using the Python transpiler
            header_content = self.transpiler.generate_header(ts_content)
            
            # Generate a basic implementation file
            base_name = ts_file.stem
            impl_content = f'''// Implementation file for {base_name}
// Auto-generated by Python transpiler backend
...
// The header file contains type definitions and declarations
...
class MultiTranspiler:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        self.directory_mappings = self.config.get('directory_mappings', {})
        self.input_path = None  # Set during transpile()
    ...
    def apply_directory_mapping(self, rel_path: Path) -> Path:
        ...
        for source_pattern, target_pattern in sorted_mappings:
            ...
            if rel_path_str.startswith(source_pattern + '/') or rel_path_str == source_pattern:
                ...
                print(f"  Mapped: {rel_path_str} -> {mapped_path}")
    ...
    def write_output(self, result: TranspilationResult, output_dir: Path, ts_file: Path):
        ...
        if result.header:
            header_file = output_dir / output_rel_path.with_suffix('.hpp')
        ...
        if result.implementation:
            impl_file = output_dir / output_rel_path.with_suffix('.cpp')
            # Create parent directories
    ...
    def transpile(self, input_path: Path, output_dir: Path, strategy: Optional[str] = None):
        """Transpile TypeScript files, preserving source directory structure"""
        # Store input path for relative path calculations in write_output
        self.input_path = input_path.resolve() if input_path.is_dir() else input_path.parent.resolve()
        ...
        if self.directory_mappings:
            print(f"Directory mappings: {len(self.directory_mappings)} configured")
        print()
