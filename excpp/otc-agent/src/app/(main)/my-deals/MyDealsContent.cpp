...
std::string formatDate(bigint tsSeconds) {
    ...
    return d.toLocaleDateString(std::nullopt, {;
        month: "short",
        day: "numeric",
        year: "numeric",
        });
...
OfferWithQuoteId transformSolanaDeal(DealFromAPI deal, const std::string& walletAddress) {
    ...
    const auto tokenAmountRaw = BigInt(;
...
std::vector<OfferWithQuoteId> mergeDealsWithOffers(const std::vector<DealFromAPI>& dbDeals, const std::vector<OfferWithQuoteId>& contractOffers, const std::string& walletAddress) {
    ...
    const auto processedOfferIds = new Set<string>();

    // Process database deals first (they have quoteId)
    for (const auto& deal : dbDeals)
...
void MyDealsContent() {
                ...
                const auto handleDisconnect = useCallback(async () => {;
                    ...
                    const auto {;
                        ...
                        const auto [refundStatus, setRefundStatus] = useState<{;
                            ...
                            const auto [myListings, setMyListings] = useState<OTCConsignment[]>([]);
                                ...
                                try {
                                    ...
                                    if (dealsRes.success && dealsRes.deals) {
                                        ...
                                        if (consignmentsRes.success) {
                                            ...
                                            std::cerr << "[MyDeals] Error fetching deals:" << error << std::endl;
                                            } finally {
                                                setIsLoadingDeals(false);
                                            }
    ...
    onClick={async () => {
        ...
        const auto now = Math.floor(Date.now() / 1000);
        const auto daysSinceCreation =;
        (now - createdAt) / (24 * 60 * 60);

        if (daysSinceCreation < 90) {
            const auto daysRemaining = Math.ceil(;
