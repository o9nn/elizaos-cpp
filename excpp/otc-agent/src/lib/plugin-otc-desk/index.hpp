 ...
 */
> tag instead of <response>. Using its content.`,
      );
...
std::future<string | null> getLatestResponseId(IAgentRuntime runtime, const std::string& roomId);:${roomId}`,
    )) ?? null
  );
}
...
std::future<void> clearLatestResponseId(IAgentRuntime runtime, const std::string& roomId);:${roomId}`;
