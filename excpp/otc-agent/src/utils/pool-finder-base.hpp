 ...
 */
async :${tokenAddress}:${baseTokenAddress}:${fee}`,
        async () => {
 ...
 */
async :${tokenAddress}:${baseTokenAddress}:${tickSpacing}`,
        async () => {
          return poolReadContract<Address>(client, {
...
async ),
  ...
  // Normalize liquidity for the estimateTVL  else {
 ...
 */
 (${pool.baseToken}) - TVL: ~$${Math.floor(pool.tvlUsd).toLocaleString()}`;
  }
