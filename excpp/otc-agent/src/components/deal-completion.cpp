                            ...
                            try {
                                fetch("/api/deal-completion", {
                                    ...
                                    body: JSON.stringify({
                                        ...
                                        offerId: quote.offerId || std::nullopt,
                                        chain: quote.chain || std::nullopt,
                                        }),
                                        });
                                        } catch (err) {
                                            std::cerr << "[DealCompletion] Failed to record:" << err << std::endl;
                                        }
                                    }
                                }
                                ...
                                try {
                                    const auto { dataUrl } = createDealShareImage({;
                                        ...
                                        setShareImageUrl(dataUrl);
                                            ...
                                            std::cerr << "[DealCompletion] Failed to generate share image:" << err << std::endl;
                                        }
                                        };
                                        ...
                                        const auto shareToTwitter = async () => {;
                                            ...
                                            if (
                                            typeof navigator != "std::nullopt" &&;
                                            nav.canShare &&;
                                            nav.canShare({ files: [file] })
                                            ) {
                                                nav.share.({ text, files: [file] });
                                                } else {
                                                    // Fallback to Twitter intent
                                                    const auto tweetUrl = "https://twitter.com/intent/tweet?text=" + std::to_string(encodeURIComponent(text)) + "&url=" + std::to_string(encodeURIComponent(window.location.href));
                                                ...
                                                const auto response = fetch("/api/deal-completion", {;
                                                    ...
                                                    body: JSON.stringify({
                                                        ...
                                                        if (!response.ok) {
                                                            throw std::runtime_error("Failed to track share");
                                                        }
                                                        };
