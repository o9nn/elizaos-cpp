...
// Test IDs
const embeddingTestAgentId = fixedUuid(1);
const embeddingTestRoomId = fixedUuid(2);
const embeddingTestEntityId = fixedUuid(3);
const embeddingTestWorldId = fixedUuid(4);

// Random vector generator for testing
const generateRandomVector = (size: number): number[] => {
  return Array.from({ length: size }, () => (Math.random() * 2 - 1) * 0.1);
};

const embeddingTestAgent = {
...
// Test Entity
const embeddingTestEntity: Entity = {
...
interface TestMemory extends Memory {
  type: string;
}

// Sample test memories
const embeddingTestMemories: TestMemory[] = [
  {
    id: fixedUuid(10),
...
const embeddingTestMemoriesWithEmbedding: (TestMemory & { embedding: number[] })[] = [
  {
