...
struct TableDefinition {
    ...
    string[]; // Tables this table depends on dependencies;
    std::optional<{ name: string; columns: string[] }; // Add composite primary key support> compositePrimaryKey;
};
...
class DrizzleSchemaIntrospector {
      ...
      for (const [index, fk] of inlineForeignKeys.entries()) {
        ...
        if (fk && fk.reference && typeof fk.reference === 'function') {
          try {
            ...
            // logger.debug(`[INTROSPECTOR] Reference );
  ...
  private extractReferencedTableName(reference: any): string | null {
    ...
    // Method 5: Check if the reference itself is a );
        }
      } catch (error) {
