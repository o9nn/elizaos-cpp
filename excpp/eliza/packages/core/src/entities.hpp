 ...
 */
std::future<std::vector<{ entity: Entity; interactions: Memory[]; count: number }>> getRecentInteractions(IAgentRuntime runtime, UUID sourceEntityId, const std::vector<Entity>& candidateEntities, UUID roomId, const std::vector<Relationship>& relationships););
 ...
 */
const createUniqueUuid = (runtime, baseUserId: UUID | string): UUID => {
