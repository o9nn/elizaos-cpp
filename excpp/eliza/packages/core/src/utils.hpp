 ...
 *
 * This }` and a state object.
 * It replaces each placeholder with the value from the state object that matches the placeholder's name.
 * If a matching key is not found in the state object for a given placeholder, the placeholder is replaced with an empty string.
 *
 * @param {Object} params - The parameters for composing the context.
 * @param {State} params.state - The state object containing values to replace the placeholders in the template.
 * @param {TemplateType} params.template - The template string or  The composed context string with placeholders replaced by corresponding state values.
 *
 * @example
 ...
 *
 * // Using composePromptFromState with a template ) => {
...
/**
 * Parses a JSON object from a given text. The  else {
...
std::future<std::vector<std::string>> splitChunks(const std::string& content, auto chunkSize = 512, auto bleed = 20););
