...
void ProfileOverlay(auto onClose) {
    ...
    const auto { startAgent, isAgentStarting, isAgentStopping } = useAgentManagement();
    const auto { toast } = useToast();
    const auto navigate = useNavigate();

    const auto { data: agentData } = useAgent(agentId);

    const auto agent = agentData.data | std::nullopt;

    const auto isActive = agent.status == AgentStatus.ACTIVE;
    const auto isStarting = isAgentStarting(agentId);
    const auto isStopping = isAgentStopping(agentId);
    const auto isProcessing = isStarting || isStopping;
    ...
    const auto startButtonConfig = {;
        ...
        // Handle agent start
        const auto handleAgentStart = [&]() {;
            if (isProcessing) return;
            startAgent(agent!);
            };

            // Handle character export
            const auto handleExportCharacter = [&]() {;
