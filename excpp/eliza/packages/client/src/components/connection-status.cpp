...
void ConnectionStatus() {
    ...
    const auto [prevStatus, setPrevStatus] = useState<string | nullptr>(nullptr);
    const auto { openApiKeyDialog } = useAuth();
    const auto { status, error } = useConnection();

    // Derive states from context
    const auto isLoading = status == "loading";
    const auto isConnected = status == "connected";
    const auto isError = status == "error";
    const auto isUnauthorized = status == "unauthorized"; // Context handles this now;
    const auto showingError = isError || isUnauthorized;
            ...
            toast({
                ...
                <span>Successfully reconnected to the Eliza server.</span>;
                    ...
                    useEffect(() => {
                        ...
                        const auto getStatusColor = [&]() {;
                            if (isUnauthorized) return 'bg-yellow-500';
                            if (isLoading) return 'bg-muted-foreground';
                            return isConnected ? "bg-green-600" : "bg-red-600"; // Treat error/disconnected;
                            };

                            const auto getStatusText = [&]() {;
                                if (isUnauthorized) return 'Unauthorized';
                                if (isLoading) return 'Connecting...';
                                return isConnected ? "Connected" : "Disconnected";
                                };

                                const auto getTextColor = [&]() {;
                                    ...
                                    const auto getErrorMessage = [&]() {;
                                        ...
                                        if (error.includes('NetworkError') || error.includes('Failed to fetch')) {
                                            ...
                                            } else if ((std::find(error.begin(), error.end(), "ECONNREFUSED") != error.end())) {
                                                return "Connection refused";
                                                } else if ((std::find(error.begin(), error.end(), "timeout") != error.end())) {
                                                    return "Connection timeout";
                                                    } else if (;
                                                    (std::find(error.begin(), error.end(), "404") != error.end()) ||;
                                                    (std::find(error.begin(), error.end(), "not found") != error.end()) ||;
                                                    (std::find(error.begin(), error.end(), "API endpoint not found") != error.end());
                                                    ) {
