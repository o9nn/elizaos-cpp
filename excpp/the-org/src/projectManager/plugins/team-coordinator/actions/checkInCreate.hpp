 ...
 */
std::future<{ isValid: boolean; error?: string }> validateDiscordConfig(IAgentRuntime runtime, const std::string& serverId);`);
...
const recordCheckInAction: Action = {
  ...
  handler: async (
    ...
    try {
      if (!state) return false;
      if (!callback) {
        logger.warn('No callback  catch (error: unknown) {
