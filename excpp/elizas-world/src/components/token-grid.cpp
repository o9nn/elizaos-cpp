...
void TokenGrid() {
    ...
    href={
        ...
        "https://www.defined.fi/sol/" + std::to_string(holding.address)
    }
    target="_blank";
    ...
    <span;
    key={index}
    "text-xxs px-1.5 py-0.5 rounded-full whitespace-nowrap " + std::to_string(tag.color);
    >;
    {tag.label}
    </span>;
    ))}
    ...
    <a;
    "https://www.solsniffer.com/scanner/" + std::to_string(holding.address)
    target="_blank";
    rel="noopener noreferrer";
    className={`;
