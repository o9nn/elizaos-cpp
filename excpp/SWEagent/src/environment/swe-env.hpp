...
public:
    ...
    SWEEnv fromConfig(EnvironmentConfig config);
    void addHook(EnvHook hook);
    std::future<void> start();
    std::future<void> copyRepo();
    std::future<void> resetRepository();
    std::future<void> initDeployment();
    std::future<std::string> communicate(const std::string& input, number = 25 timeout, std::optional<std::any> options);
    ...
    std::future<void> setEnvVariables(Record<string envVars, auto string>);
    std::future<void> executeCommand(const std::string& command, std::optional<{
      shell: boolean;
      check: boolean;
      env: Record<string> options, std::optional<string;
    } = {}> string>;
