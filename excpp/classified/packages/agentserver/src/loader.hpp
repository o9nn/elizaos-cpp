 ...
 */
std::future<std::vector<Character>> loadCharactersFromUrl(const std::string& url);: ${response.statusText}`);
  }
 ...
 */
std::future<Character> jsonToCharacter(unknown character);: ${issue.message}`)
          .join('; ')
