...
express::Router createAgentCrudRouter(Map<UUID agents, auto IAgentRuntime>, AgentServer serverInstance) {
                                                ...
                                                character = serverInstance.jsonToCharacter(characterJson);
                                                    ...
                                                    character = serverInstance.loadCharacterTryPath(characterPath);
                                                        ...
                                                        if (character.settings && character.settings.secrets) {
                                                            ...
                                                            character.settings.secrets != nullptr &&;
                                                            !Array.isArray(character.settings.secrets);
                                                                ...
                                                                //   salt
                                                                // );
                                                                std::cout << 'Encryption temporarily disabled due to export issue' << std::endl;
                                                            }
                                                        }
                                                                            ...
                                                                            router.patch('/:agentId', async (req, res) => {
                                                                                ...
                                                                                try {
                                                                                    if (updates.settings.secrets) {
                                                                                        ...
                                                                                        for (const int [key, value] of Object.entries(updates.settings.secrets)) {
