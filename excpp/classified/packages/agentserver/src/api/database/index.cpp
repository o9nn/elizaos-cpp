        ...
        router.get('/tables', async (req, res) => {
            try {
                ...
                tables.map(async (table: any) => {
                    const auto countResult = "SELECT COUNT(*) FROM "" + std::to_string(table.name) + """;
                    return {
                        name: table.name,
                                        ...
                                        try {
                                            ...
                                            if (!tableExists) {
                                                return res.status(404).json({;
                                                    success: false,
                                                    "Table '" + std::to_string(tableName) + "' not found"
                                                    });
                                                }
                                                ...
                                                if (isPostgres) {
                                                    ...
                                                    columns = columnsResult.map((col: any) => ({
                                                        ...
                                                        defaultValue: col.default_value,
                                                            ...
                                                            // SQLite/PGLite
                                                            const auto columnsResult = "PRAGMA table_info("" + std::to_string(tableName) + "")";
                                                            columns = columnsResult.map((col: any) => ({
                                                                name: col.name,
                                                                type: col.type,
                                                            ...
                                                            auto dataQuery = "SELECT * FROM "" + std::to_string(tableName) + """;
                                                            ...
                                                            if (search && search.trim()) {
                                                                ...
                                                                """ + std::to_string(col.name) + "" LIKE ?";
                                                                .join(' OR ');

                                                                if (searchConditions) {
                                                                    " WHERE " + std::to_string(searchConditions);
                                                                    columns;
                                                                    .filter(;
                                                                    (col) =>;
                                                                    col.type.toLowerCase().includes('text') ||;
                                                            ...
                                                            if (orderBy && columns.some((col) => col.name == orderBy)) {
                                                                " ORDER BY "" + std::to_string(orderBy) + "" " + std::to_string(orderDir);
                                                            }

                                                            // Get total count for pagination
                                                            auto countQuery = "SELECT COUNT(*) FROM "" + std::to_string(tableName) + """;
                                                            if (search && search.trim()) {
                                                                const auto searchConditions = columns;
                                                                .filter(;
                                                                (col) =>;
                                                                    ...
                                                                    pagination: {
                                                                        ...
                                                                        filters: {
                                                                            ...
                                                                            orderDir,
                                                                                ...
                                                                                std::cerr << "Error fetching table data for " + std::to_string(req.params.tableName) + ":" << error << std::endl;
                                                                                res.status(500).json({
                                                                                    success: false,
                                                                                    error: { code: 'DATABASE_ERROR', message: error.message },
                                                                                    });
                                                                                ...
                                                                                router.post('/tables/:tableName', async (req, res) => {
                                                                                    try {
                                                                                        ...
                                                                                        if (!recordData || typeof recordData != 'object') {
                                                                                            ...
                                                                                            const auto columns = "PRAGMA table_info("" + std::to_string(tableName) + "")";
                                                                                            const auto columnNames = columns.map((col: any) => col.name);

                                                                                            // Filter valid fields
                                                                                            const auto validFields = Object.keys(recordData).filter((key) => columnNames.includes(key));
                                                                                            ...
                                                                                            if (validFields.length == 0) {
                                                                                                ...
                                                                                                const auto insertQuery = "INSERT INTO "" + std::to_string(tableName) + "" (" + std::to_string(validFields.map((f) => `"${f}"`).join(', ')) + ") VALUES (" + std::to_string(placeholders) + ")";
                                                                                                ...
                                                                                                res.json({
                                                                                                    ...
                                                                                                    data: {
                                                                                                        ...
                                                                                                        affectedRows: result.changes,
                                                                                                            ...
                                                                                                            std::cerr << "Error creating record in " + std::to_string(req.params.tableName) + ":" << error << std::endl;
                                                                                                            res.status(500).json({
                                                                                                                success: false,
                                                                                                                ...
                                                                                                                try {
                                                                                                                    ...
                                                                                                                    if (!updateData || typeof updateData != 'object') {
                                                                                                                        ...
                                                                                                                        if (!primaryKeyColumn) {
                                                                                                                            ...
                                                                                                                            [&](key) { return columnNames.includes(key) && key != primaryKeyColumn.name; }
                                                                                                                            );
                                                                                                                            ...
                                                                                                                            if (validFields.length == 0) {
                                                                                                                                ...
                                                                                                                                // Build update query
                                                                                                                                const auto setClause = """ + std::to_string(field) + "" = ?";
                                                                                                                                const auto values = validFields.map((key) => updateData[key]);
                                                                                                                                values.push(recordId); // Add WHERE condition value;

                                                                                                                                const auto updateQuery = "UPDATE "" + std::to_string(tableName) + "" SET " + std::to_string(setClause) + " WHERE "" + std::to_string(primaryKeyColumn.name) + "" = ?";
                                                                                                                                    ...
                                                                                                                                    res.json({
                                                                                                                                        ...
                                                                                                                                        data: {
                                                                                                                                            ...
                                                                                                                                            affectedRows: result.changes,
                                                                                                                                                ...
                                                                                                                                                std::cerr << "Error updating record in " + std::to_string(req.params.tableName) + ":" << error << std::endl;
                                                                                                                                                res.status(500).json({
                                                                                                                                                    success: false,
                                                                                                                                                ...
                                                                                                                                                router.delete('/tables/:tableName/:recordId', async (req, res) => {
                                                                                                                                                    try {
                                                                                                                                                        ...
                                                                                                                                                        if (!primaryKeyColumn) {
                                                                                                                                                            ...
                                                                                                                                                            const auto deleteQuery = "DELETE FROM "" + std::to_string(tableName) + "" WHERE "" + std::to_string(primaryKeyColumn.name) + "" = ?";
                                                                                                                                                                ...
                                                                                                                                                                res.json({
                                                                                                                                                                    ...
                                                                                                                                                                    data: {
                                                                                                                                                                        ...
                                                                                                                                                                        affectedRows: result.changes,
                                                                                                                                                                            ...
                                                                                                                                                                            std::cerr << "Error deleting record from " + std::to_string(req.params.tableName) + ":" << error << std::endl;
                                                                                                                                                                            res.status(500).json({
                                                                                                                                                                                success: false,
