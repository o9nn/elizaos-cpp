 ...
 */
std::future<bool> checkProviderValidity(IAgentRuntime runtime, InferenceProvider providerName, ProviderInfo providerInfo);-validation`;
 ...
 */
std::future<{ name: string; plugin: Plugin } | null> getActiveProvider(IAgentRuntime runtime);;
    } else {
      logger.warn(
...
default inferencePlugin;

// Export Claude handler for external use
{ createClaudeHandler, ClaudeProxy } from './claude-proxy';
