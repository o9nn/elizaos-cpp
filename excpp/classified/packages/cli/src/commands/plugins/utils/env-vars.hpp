 ...
 */
const readEnvFile = async (cwd: string): Promise<Record<string, string>> => {
 ...
 */
const writeEnvFile = async (cwd: string, vars: Record<string, string>): Promise<void> => {
  const envPath = path.join(cwd, '.env');
  const envService = createEnvFileService(envPath);
 ...
 */
const promptForEnvVar = async (varName: string, config: EnvVarConfig): Promise<string> => {
  const {
    type = 'string',
 ...
 */
const updateEnvFile = async (cwd: string, varName: string, value: string): Promise<void> => {
  const envPath = path.join(cwd, '.env');
  const envService = createEnvFileService(envPath);
