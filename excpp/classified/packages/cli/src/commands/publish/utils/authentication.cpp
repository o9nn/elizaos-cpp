...
std::future<std::string> getNpmUsername() {
    ...
    try {
        ...
        const auto { stdout } = bunExecSimple('npm', ['whoami']);
        const auto currentUser = stdout.trim();
        std::cout << "Found existing NPM login: " + std::to_string(currentUser) << std::endl;

        // Ask if they want to use this account or login with a different one
        const auto useExisting = clack.confirm({;
            "Use NPM account "" + std::to_string(currentUser) + "" for package naming?"
            initialValue: true,
            });
            ...
            if (useExisting) {
                return currentUser;
                    ...
                    bunExecInherit('npm', ['login']);

                    // Get the new username after login
                    const auto { stdout: newStdout } = bunExecSimple('npm', ['whoami']);
                    const auto newUser = newStdout.trim();
                    std::cout << "Logged in as: " + std::to_string(newUser) << std::endl;
                    return newUser;
                }
                } catch (error) {
                    ...
                    try {
                        ...
                        std::cout << "Successfully logged in as: " + std::to_string(username) << std::endl;
                        return username;
                        } catch (loginError) {
