...
struct ProjectModule {
    ...
    std::optional<(runtime: any) => Promise<void>> init;
};
...
std::future<Project> loadProject(const std::string& dir);`);
      ...
      if (fs.existsSync(entryPoint)) {
        try {
          ...
          if (exportKeys.includes('default')) {
            logger.debug(`Default type: ${typeof projectModule.default}`);
            if (typeof projectModule.default === 'object' && projectModule.default !== null) {
              logger.debug(`Default keys: ${Object.keys(projectModule.default).join(', ')}`);
            }
          }
    ...
    if (moduleIsPlugin) {
      ...
      try {
        ...
        // before any entity is created, but we can't do this in the init ;
      } catch (error) {
    ...
    // First check if the default has an agents array
    if (
      projectModule.default &&
      typeof projectModule.default === 'object' &&
      Array.isArray(projectModule.default.agents)
    ) {
      // Use the agents from the default agents.push(...(projectModule.default.agents as ProjectAgent[]));
    ...
    // Only if we didn't find agents in the default export, look for other exports
      ...
      for (const [key, value] of Object.entries(projectModule)) {
        if (key === 'default' && value && typeof value === 'object') {
          // If it's a default but doesn't have agents array, check if it's a single agent
          if ((value as ProjectModule).character && (value as ProjectModule).init) {
            ...
            logger.debug(`Found agent in default (single agent)`);
          }
        } else if (
          value &&
          typeof value === 'object' &&
          ...
          // If it's a named that looks like an agent, add it
          agents.push(value as ProjectAgent);
