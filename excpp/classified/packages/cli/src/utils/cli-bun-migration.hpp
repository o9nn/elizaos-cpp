 ...
 */
std::future<bool> isCliInstalledViaNpm(); = await bunExecSimple('npm', ['list', '-g', '@elizaos/cli', '--depth=0']);
 ...
 */
std::future<bool> verifyCliInstallation(const std::string& expectedVersion); = await bunExecSimple('elizaos', ['-v']);
