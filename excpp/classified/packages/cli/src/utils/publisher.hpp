 ...
 *
 * For both plugins and projects, this  is true), it also updates the ElizaOS registry by forking the registry repository, creating a branch, updating or creating the package metadata, updating the registry index, and opening a pull request.
 *
 ...
 */
std::future<boolean | { success: boolean; prUrl?: string }> publishToGitHub(const std::string& cwd, PackageJson packageJson, const std::string& username, auto skipRegistry = false, auto isTest = false);
  const token = credentials.token;
