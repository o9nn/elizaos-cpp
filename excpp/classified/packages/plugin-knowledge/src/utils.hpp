 ...
 */
std::future<std::string> extractTextFromFileBuffer(Buffer fileBuffer, const std::string& contentType, string // For logging and context originalFilename); (type: ${contentType})`
  );
 ...
 */
std::future<{ content: string; contentType: string }> fetchUrlContent(const std::string& url);`);
