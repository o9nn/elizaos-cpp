 ...
 */
const goalRelations = relations(goalsTable, ({ many }) => ({
  tags: many(goalTagsTable),
}));

const goalTagRelations = relations(goalTagsTable, ({ one }) => ({
  goal: one(goalsTable, {
