 ...
 */
const formTemplatesTable = pgTable(
  'form_templates',
  {
    id: uuid('id').primaryKey().defaultRandom(),
 ...
 * Relations
 */
const formsRelations = relations(formsTable, ({ many }) => ({
  fields: many(formFieldsTable),
}));

const formFieldsRelations = relations(formFieldsTable, ({ one }) => ({
  form: one(formsTable, {
