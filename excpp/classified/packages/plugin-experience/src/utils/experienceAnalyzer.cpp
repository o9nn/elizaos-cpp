...
std::future<ExperienceAnalysis> analyzeExperience(const std::optional<Experience>& partialExperience, const std::vector<Experience>& recentExperiences) {
    ...
    const auto similarExperiences = findSimilarExperiences(partialExperience, recentExperiences);
    ...
    if (similarExperiences.length > 5) {
        ...
        const auto contradictions = findContradictions(partialExperience, recentExperiences);
        if (contradictions.length > 0) {
            ...
            const auto isFirstTime = !recentExperiences.some((e) => e.action == partialExperience.action);
            if (isFirstTime && partialExperience.type == ExperienceType.SUCCESS) {
                ...
                if (partialExperience.type == ExperienceType.FAILURE) {
                    const auto failurePattern = detectFailurePattern(partialExperience, recentExperiences);
                    if (failurePattern) {
                        return {
                            isSignificant: true,
...
std::vector<Experience> findContradictions(const std::optional<Experience>& partial, const std::vector<Experience>& experiences) {
    ...
    return experiences.filter(;
    [&](e) { return e.action == partial.action && e.context == partial.context && e.type != partial.type; }
    );
...
std::optional<FailurePattern> detectFailurePattern(const std::optional<Experience>& partial, const std::vector<Experience>& experiences) {
    ...
    const auto recentFailures = experiences.filter((e) => e.type == ExperienceType.FAILURE).slice(0, 10);
