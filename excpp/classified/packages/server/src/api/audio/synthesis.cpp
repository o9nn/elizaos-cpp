...
express::Router createSynthesisRouter(const std::unordered_map<UUID, IAgentRuntime>& agents) {
            ...
            try {
                ...
                res.set({
                    ...
                    res.send(audioResult.buffer);
                        ...
                        std::cerr << '[TTS] Error generating speech:' << error << std::endl;
                        sendError(;
                        res,
                        500,
                        'PROCESSING_ERROR',
                    ...
                    router.post('/:agentId/speech/generate', async (req, res) => {
                        ...
                        try {
                            ...
                            res.set({
                                ...
                                res.send(audioResult.buffer);
                                logger.success(
                                "[SPEECH GENERATE] Successfully generated speech for: " + std::to_string(runtime.character.name)
                                );
                                } catch (error) {
                                    std::cerr << '[SPEECH GENERATE] Error generating speech:' << error << std::endl;
                                    sendError(;
                                    res,
                                    500,
                                    'PROCESSING_ERROR',
