...
`;
  ...
  private async cloneExistingPlugin(projectPath: string, prd: PRDDocument): Promise<void> {
    ...
    if (!clonePlugin) {
      throw new Error('Plugin registry clone `);
    }
...
The file should a valid ElizaOS plugin with all required properties.`;
...
interface ${request.description.toLowerCase().includes('weather') ? 'WeatherData' : 'ApiResponse'} {
${
