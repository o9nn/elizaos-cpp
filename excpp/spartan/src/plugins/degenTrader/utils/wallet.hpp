...
}> executeTrade(IAgentRuntime runtime, {
    tokenAddress: string;
    amount: string;
    slippage: number;
    dex: string;
    action: string;
  } params); trade using ${params.dex}:`, {
...
std::future<{ success: boolean; signature?: string; error?: string }> executeRaydiumTrade(IAgentRuntime runtime, std::optional<{
    tokenAddress: string;
    amount: string;
    slippage: number;
    isSell: boolean;
