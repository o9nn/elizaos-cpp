 ...
 */
std::future<string | null> resolveTokenSymbol(const std::string& symbol, const std::string& network); on ${network}: ${hardcodedAddress}`);
 ...
 */
std::future<`0x${string}` | null> resolveTokenToAddress(const std::string& token, const std::string& network); on network: ${network}`);
