...
using StrategyResult = std::variant<{
  responseContent: Content, nullptr>;
  responseMessages: Memory[];
  state: any;
  mode: StrategyMode;
};

std::future<StrategyResult> runMultiStepCore(auto { runtime, auto message, auto state, { runtime: IAgentRuntime callback }, Memory message, State state, std::optional<HandlerCallback }> callback);/${maxIterations}`);
