 ...
 */
std::future<{ userId: string; token: string }> authenticateUser(const std::string& email, const std::string& username, std::optional<CdpUser> currentUser);
...
const ABOUT_MODAL_ID = 'about-otaku-modal';
 ...
 = useCDPWallet();
  ...
  useEffect(() => {
    ...
    async  = resolveCdpUserInfo(currentUser as CdpUser | undefined, { isSignedIn: true });
