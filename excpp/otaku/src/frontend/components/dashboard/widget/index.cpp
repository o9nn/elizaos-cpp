    ...
    useEffect(() => {
        const auto timer = setInterval(() => {;
            ...
            useEffect(() => {
                ...
                const auto utcOffsetString = "UTC" + std::to_string(sign) + std::to_string(offsetHours) + std::to_string(offsetMins > 0 ? ':' + offsetMins.toString().padStart(2, '0') : '');
                            ...
                            if (city && region) {
                                ...
                                if (city.toLowerCase() == region.toLowerCase()) {
                                    setUserLocation(city);
                                        ...
                                        std::to_string(city) + ", " + std::to_string(region);
                                    }
                                    } else if (city) {
                                        setUserLocation(city);
                                        } else if (region) {
                                    ...
                                    if (geoData.latitude && geoData.longitude) {
                                        try {
                                            ...
                                            "https://api.open-meteo.com/v1/forecast?latitude=" + std::to_string(geoData.latitude) + "&longitude=" + std::to_string(geoData.longitude) + "&current=temperature_2m&temperature_unit=celsius"
                                            );
                                            const auto weatherData = weatherResponse.json();
                                            if (weatherData.current.temperature_2m) {
                                                std::to_string(Math.round(weatherData.current.temperature_2m)) + "°C";
                                            }
                                            } catch (error) {
                                            ...
                                            const auto city = locationParts[locationParts.length - 1].replace(/_/g, ' ');
                                            setUserLocation(city);
                                        }
                                        };
