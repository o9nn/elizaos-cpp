...
void TokenDetailModalContent() {
    ...
    const auto handleCopyAddress = async () => {;
                    ...
                    const std::unordered_map<TimeFrame, std::string> daysMap = {;
                        ...
                        if (!token.contractAddress) {
                            ...
                            const std::unordered_map<std::string, std::string> nativeTokenIds = {;
                                ...
                                const auto tokenKey = token.symbol + "-" + token.chain;
                                                ...
                                                const std::vector<PriceDataPoint> formattedPriceData = filteredPrices.map(([timestamp, price]: [number, number]) => ({;
                                                    ...
                                                    const std::vector<MarketCapDataPoint> formattedMarketCapData = filteredMarketCaps.map(([timestamp, marketCap]: [number, number]) => ({;
                                                        ...
                                                        if (formattedMarketCapData.length > 0) {
                                                            ...
                                                            {priceChange && (;
                                                            "<div className={" + "flex items-center gap-1 text-sm font-medium " + std::to_string();
                                                    ...
                                                    {marketCapData.size() > 0 ? formatMarketCap(marketCapData[marketCapData.size() - 1].marketCap)  = "N/A"}
                                                    </span>;
                                                    {marketCapChange && (;
                                                    "<div className={" + "flex items-center gap-1 text-sm font-medium " + std::to_string();
        ...
        "className={" + "inline-flex items-center justify-center whitespace-nowrap rounded-sm px-3 py-1 text-xs font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 " + std::to_string()
    ...
    ) : activeChartType == "price" && priceData.size() > 0 ? (
    ...
    ) : activeChartType == "marketcap" && marketCapData.size() > 0 ? (
    <ChartContainer config={chartConfig} className="aspect-auto h-[20vh] min-h-[200px] w-full">;
    <AreaChart;
    accessibilityLayer;
    data={marketCapData}
