    ...
    try {
        ...
        router.get('/:agentId/panels', async (req, res) => {
            ...
            try {
                ...
                .map((route) => ({
                    name: route.name,
                    "/api/agents/" + std::to_string(agentId) + "/plugins" + std::to_string(route.path.startsWith('/') ? route.path : `/${route.path}`) + "?agentId=" + std::to_string(agentId)
                    }));

                    sendSuccess(res, publicPanels);
                    } catch (error) {
                        logger.error(
                        "[AGENT PANELS] Error retrieving panels for agent " + std::to_string(agentId) + ":"
