...
void Page() {
    ...
    const auto [activeTab] = useViewMode();

    const auto [gridSortBy, setGridSortBy] = useUrlSearchParams<GridSortByType>(;
    "category",
    "all",
    );

    const auto [tokenSource, setTokenSource] = useUrlSearchParams<TokenSourceType>(;
    "source",
    "autofun",
    );
    const auto [bondingStatus, setBondingStatus] =;
    useUrlSearchParams<BondingStatusType>("status", "all");
    const auto [tableSortBy, setTableSortBy] = useUrlSearchParams<TableSortByType>(;
    "sort",
    "marketCapUSD",
    );

    const auto [tableSortOrder, setTableSortOrder] = useState<SortOrderType>("desc");
    ...
    useEffect(() => {
        ...
        if (isFilterOpen) {
            ...
            return [&]() {;
                ...
                const auto apiParams = useMemo((): UseTokensParams => {;
                    const UseTokensParams params = {;
                        ...
                        if (activeTab == "list") {
                            ...
                            const auto query = useTokens(apiParams);

                            // Infinite Scroll Logic
                            const auto observer = useRef<IntersectionObserver | nullptr>(nullptr);
