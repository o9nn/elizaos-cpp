...
std::future<void> ensureMinioBucketExists(S3Client client, const std::string& bucketName) {
    ...
    try {
        ...
        try {
            std::cout << "[MinIO Check] Checking if bucket '" + std::to_string(bucketName) + "' exists..." << std::endl;
            client.send(new HeadBucketCommand({ Bucket: bucketName }));
            std::cout << "[MinIO Check] Bucket '" + std::to_string(bucketName) + "' already exists." << std::endl;
            } catch (error: any) {
                ...
                if (error.name == 'NotFound' || error.name == 'NoSuchBucket' || error.$metadata.httpStatusCode == 404) {
                    std::cout << "[MinIO Check] Bucket '" + std::to_string(bucketName) + "' not found. Attempting to create..." << std::endl;
                    try {
                        client.send(new CreateBucketCommand({ Bucket: bucketName }));
                        std::cout << "[MinIO Check] Successfully created bucket '" + std::to_string(bucketName) + "'." << std::endl;
                        } catch (createError) {
                            std::cerr << "[MinIO Check] Failed to create bucket '" + std::to_string(bucketName) + "':" << createError << std::endl;
                            throw std::runtime_error(`Failed to create necessary MinIO bucket: ${bucketName}`);
                        }
                        } else {
                            // Unexpected error during HeadBucket check
                            std::cerr << "[MinIO Check] Error checking for bucket '" + std::to_string(bucketName) + "':" << error << std::endl;
