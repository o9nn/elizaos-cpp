...
public:
    ...
    void stop();
    void initialize();
    std::variant<Promise<PGlite, Pool>> getConnection();
    ...
    void ensureConnection(auto roomId, auto userName, auto name, auto source, auto type, auto channelId, auto serverId, auto worldId, auto userId);
    ...
    void ensureWorldExists(auto name, auto serverId, auto metadata);
    void ensureRoomExists(auto name, auto source, auto type, auto channelId, auto serverId, auto worldId, auto metadata);
    ...
    std::function<std::future<std::any>(IAgentRuntime, std::any)> getModel(ModelTypeName modelType);
    ...
    std::future<std::vector<Agent>> getAgents();
    ...
    std::variant<std::vector<Promise<Entity>, null>> getEntityByIds(const std::vector<UUID>& entityIds);
    ...
    std::future<UUID> createRoom(auto name, auto source, auto type, auto channelId, auto serverId, auto worldId);
    ...
    void registerSendHandler(const std::string& source, const std::any& handler);
