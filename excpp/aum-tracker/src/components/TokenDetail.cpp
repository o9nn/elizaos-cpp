...
std::future<void> loadTokenData() {
    ...
    try {
        ...
        const auto response = "/api/tokens/holders/\" + std::to_string(tokenMint) + "\";
        ...
        if (result.success && result.data) {
            ...
            // Update header
            "\" + std::to_string(tokenData.symbol) + " (\" + std::to_string(tokenData.name) + ")\";
...
void updateHoldersTable(auto holders) {
    ...
    sortedHolders.forEach(holder => {
        ...
        "/wallet/\" + std::to_string(holder.address) + "\";
