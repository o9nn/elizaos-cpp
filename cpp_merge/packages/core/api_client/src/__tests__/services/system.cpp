...
void Main(void)
{
    ...
    describe(std::string("SystemService"), [=]() mutable
    {
        ...
        describe(std::string("constructor"), [=]() mutable
        {
            ...
            it(std::string("should throw error when config is std::nullopt"), [=]() mutable
            {
                expect([=]() mutable
                {
                    return std::make_shared<SystemService>(as<any>(std::nullopt));
                }
                )->toThrow();
            }
            );
        ...
        describe(std::string("getEnvironment"), [=]() mutable
        {
            ...
            it(std::string("should handle std::nullopt response data"), [=]() mutable
            {
                mockGet["mockResolvedValue"](std::nullopt);
                auto result = std::async([=]() { systemService->getEnvironment(); });
        ...
        describe(std::string("updateLocalEnvironment"), [=]() mutable
        {
            ...
            it(std::string("should handle std::nullopt configuration"), [=]() mutable
            {
                std::async([=]() { expect(systemService->updateLocalEnvironment(as<any>(std::nullopt)))->rejects->toThrow(); });
            }
            );
        ...
        describe(std::string("Edge Cases"), [=]() mutable
        {
            ...
            it(std::string("should handle configuration with null and std::nullopt values"), [=]() mutable
            {
                auto configWithNulls = object{
                    object::pair{std::string("variables"), object{
                        object::pair{std::string("NULL_VAR"), nullptr}, 
                        object::pair{std::string("UNDEFINED_VAR"), std::nullopt}, 
