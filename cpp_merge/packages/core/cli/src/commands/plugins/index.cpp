#include "/home/runner/work/elizaos-cpp/elizaos-cpp/eliza/packages/cli/src/commands/plugins/index.h"
...
void Main(void)
{
    ...
    plugins->command(std::string("upgrade"))->description(std::string("Upgrade a plugin from version 0.x to 1.x using AI-powered migration"))->argument(std::string("<path>"), std::string("GitHub repository URL or local folder path"))->option(std::string("--api-key <key>"), std::string("Anthropic API key (or use ANTHROPIC_API_KEY env var)"))->option(std::string("--skip-tests"), std::string("Skip test validation loop"))->option(std::string("--skip-validation"), std::string("Skip production readiness validation"))->action([=](auto pluginPath, auto opts) mutable
    {
