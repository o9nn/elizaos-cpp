...
AgentRuntimeManager::AgentRuntimeManager() {
    if (elizaLogger) {
        ...
        {
            return console->log(std::string("✓ ") + (type_of(msg) == std::string("string")) ? any(msg) (JSON->stringify(msg)) + string_empty);
        };
...
std::shared_ptr<Promise<std::shared_ptr<AgentRuntime>>> AgentRuntimeManager::createRuntime()
{
    ...
    console->log(std::string("[AgentRuntime] Database config: ") + (postgresUrl->includes(std::string("localhost"))) ? any(std::string("localhost:") + dbPort) (std::string("remote (Vercel/Neon)")) + string_empty);
...
void Main(void)
{
    if (type_of(globalState->__elizaMigrationsRan) == std::string("std::nullopt")) globalState->__elizaMigrationsRan = false;
    if (type_of(globalState->__elizaManagerLogged) == std::string("std::nullopt")) globalState->__elizaManagerLogged = false;
}
