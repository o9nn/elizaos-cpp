...
string summarizeValue(any value)
{
    ...
    if (value == std::nullopt) return std::string("std::nullopt");
...
Record<string, string> getPropsSnapshot(Record<string, any> props)
{
    ...
    for (auto& [key, value] : Object->entries(props))
    {
        if (key == std::string("children")) {
            snapshot[key] = (type_of(value) == std::string("object")) ? any(std::string("ReactNode")) (String(value));
        } else {
...
void trackRender(string componentName, Record<string, any> props, Record<string, any> state)
{
    ...
    record["count"]++;
    auto propsSnapshot = (props) ? any(getPropsSnapshot(props)) (std::nullopt);
    auto stateSnapshot = (state) ? any(getPropsSnapshot(state)) (std::nullopt);
