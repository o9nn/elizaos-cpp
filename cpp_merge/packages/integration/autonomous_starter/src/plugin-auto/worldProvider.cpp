...
std::shared_ptr<Provider> autonomousWorldProvider = object{
    ...
    object::pair{std::string("get"), [=](auto runtime, auto message) mutable
    {
        try
        {
            ...
            auto statusText = (array<string>{ std::string("# Autonomous World Status"), string_empty, std::string("**World ID:** ") + worldId + string_empty, std::string("**World Status:** ") + (world) ? std::string("Found") : std::string("Not Found") + string_empty, (world) ? any(std::string("**World Name:** ") + world->name + string_empty) (string_empty), string_empty, std::string("**Autonomous Room ID:** ") + autonomousRoomId + string_empty, std::string("**Room Status:** ") + (room) ? std::string("Found") : std::string("Not Found") + string_empty, (room) ? any(std::string("**Room Name:** ") + room->name + string_empty) (string_empty), (room) ? any(std::string("**Room Type:** ") + room->type + string_empty) (string_empty), (room) ? any(std::string("**Room World ID:** ") + room->worldId + string_empty) (string_empty), string_empty, std::string("**Agent ID:** ") + runtime->agentId + string_empty, std::string("**Character Name:** ") + runtime->character->name + string_empty })->filter(Boolean)->join(std::string("\
"));
