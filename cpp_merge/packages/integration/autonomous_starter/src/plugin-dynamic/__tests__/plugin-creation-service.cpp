#include "plugin-creation-service.test.h"
#include <string>
...
void Main(void)
{
    ...
    describe(std::string("PluginCreationService"), [=]() mutable
    {
        ...
        beforeEach([=]() mutable
        {
            ...
            mockFs = as<any>(fs);
            vi->mocked(fs->ensureDir)->mockResolvedValue(std::nullopt);
            vi->mocked(fs->writeJson)->mockResolvedValue(std::nullopt);
            vi->mocked(fs->writeFile)->mockResolvedValue(std::nullopt);
            vi->mocked(fs->remove)->mockResolvedValue(std::nullopt);
