...
{
    auto config = getChainConfig(chain);
    return (config) ? any(string_empty + config->explorerUrl + std::string("/tx/") + txHash + string_empty) (nullptr);
};
...
{
    auto config = getChainConfig(chain);
    return (config) ? any(string_empty + config->explorerUrl + std::string("/address/") + address + string_empty) (nullptr);
};
