#include "action-message-utils.h"
#include <string>
...
std::shared_ptr<ToolPart> convertActionMessageToToolPart(any message)
{
    ...
    auto errorText = (isError) ? any(OR((OR((rawMessage["text"]), (message["content"]))), (std::string("Action failed")))) (std::nullopt);
    return object{
        ...
        object::pair{std::string("input"), (Object->keys(inputData)->get_length() > 0) ? any(inputData) (std::nullopt)}, 
        object::pair{std::string("output"), (Object->keys(outputData)->get_length() > 0) ? any(outputData) (std::nullopt)}, 
