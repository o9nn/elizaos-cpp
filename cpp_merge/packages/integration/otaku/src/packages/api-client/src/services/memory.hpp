#ifndef _HOME_RUNNER_WORK_ELIZAOS_CPP_ELIZAOS_CPP_OTAKU_SRC_PACKAGES_API_CLIENT_SRC_SERVICES_MEMORY_H
#define _HOME_RUNNER_WORK_ELIZAOS_CPP_ELIZAOS_CPP_OTAKU_SRC_PACKAGES_API_CLIENT_SRC_SERVICES_MEMORY_H
#include "core.hpp"
...
public:
    ...
    virtual std::shared_ptr<Promise<object>> getAgentMemories(std::shared_ptr<UUID> agentId, std::shared_ptr<MemoryParams> params = std::nullopt);
    virtual std::shared_ptr<Promise<object>> getRoomMemories(std::shared_ptr<UUID> agentId, std::shared_ptr<UUID> roomId, std::shared_ptr<MemoryParams> params = std::nullopt);
    ...
    virtual std::shared_ptr<Promise<object>> getAgentInternalMemories(std::shared_ptr<UUID> agentId, std::shared_ptr<UUID> agentPerspectiveRoomId, boolean includeEmbedding = std::nullopt);
