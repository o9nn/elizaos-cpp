                        ...
                        catch (const any& error)
                        {
                            ...
                            if (!res["headersSent"]) {
                                ...
                                res["status"](status)["json"](object{
                                    object::pair{std::string("error"), (is<Error>(error)) ? any(error->message) (std::string("Error processing wildcard route"))}
                                });
                            }
                            handled = true;
                        }
                    ...
                    if (matched) {
                        ...
                        catch (const any& error)
                        {
                            ...
                            if (!res["headersSent"]) {
                                auto status = (OR(((AND((AND((is<Error>(error)), (in(std::string("code"), error)))), (error->code == std::string("ENOENT"))))), ((AND((is<Error>(error)), (error->message->includes(std::string("not found")))))))) ? 404 : 500;
                                res["status"](status)["json"](object{
                                    object::pair{std::string("error"), (is<Error>(error)) ? any(error->message) (std::string("Error processing route"))}
                                });
                            }
                            handled = true;
                        }
