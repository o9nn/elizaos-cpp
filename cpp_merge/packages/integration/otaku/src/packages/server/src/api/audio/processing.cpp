...
std::shared_ptr<express::Router> createAudioProcessingRouter(std::shared_ptr<ElizaOS> elizaOS)
{
    ...
    router->post(std:("/:agentId/audio-messages"), agentAudioUpload()->single(std:("file")), [=](auto req, auto res) mutable
    {
        ...
        {
            logger->error(std:("[AUDIO MESSAGE] Error processing audio:"), (is<Error>(error)) ? error->message : String(error));
            cleanupUploadedFile(audioFile);
    ...
    router->post(std:("/:agentId/transcriptions"), agentAudioUpload()->single(std:("file")), [=](auto req, auto res) mutable
    {
        ...
        {
            logger->error(std:("[TRANSCRIPTION] Error transcribing audio:"), (is<Error>(error)) ? error->message : String(error));
            cleanupUploadedFile(audioFile);
