...
std::shared_ptr<Action> cdpTxExplorerLink = object{
    ...
    object::pair{std::string("handler"), [=](auto runtime, auto message, auto state = std::nullopt, auto _options = std::nullopt, auto callback = std::nullopt) mutable
    {
        try
        {
            ...
            auto inputParams = object{
                object::pair{std::string("txhash"), OR((txhash), (std::nullopt))}, 
                object::pair{std::string("network"), OR((network), (std::nullopt))}
            };
            if (!txhash) {
