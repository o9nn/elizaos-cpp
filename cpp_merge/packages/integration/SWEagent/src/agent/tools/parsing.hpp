...
public:
    ...
    std::tuple<string, string> call(P0 modelResponse, array<std::shared_ptr<Command>> _commands = std::nullopt, boolean strict = std::nullopt) = 0;
    template <typename P0>
    std::tuple<string, string> parse(P0 modelResponse, array<std::shared_ptr<Command>> commands = std::nullopt, boolean strict = true);
};
...
public:
    ...
    template <typename P0>
    std::tuple<string, string> call(P0 modelResponse, array<std::shared_ptr<Command>> _commands = std::nullopt, boolean strict = false);
};
...
public:
    ...
    template <typename P0>
    std::tuple<string, string> call(P0 modelResponse, array<std::shared_ptr<Command>> _commands = std::nullopt, boolean _strict = false);
};
...
std::tuple<string, string> parseAction(P0 modelResponse, P1 parseFunction = std::string("thought_action"), array<std::shared_ptr<Command>> commands = std::nullopt, boolean strict = false);
...
public:
    ...
    template <typename P0>
    std::tuple<string, string> call(P0 modelResponse, array<std::shared_ptr<Command>> _commands = std::nullopt, boolean _strict = std::nullopt);
};
...
public:
    ...
    template <typename P0>
    std::tuple<string, string> call(P0 modelResponse, array<std::shared_ptr<Command>> _commands = std::nullopt, boolean strict = std::nullopt);
};
...
{
    auto toolCalls = (type_of(modelResponse) != std::string("string")) ? any(OR((modelResponse->toolCalls), ((as<any>(modelResponse))["tool_calls"]))) (nullptr);
    if (AND((toolCalls), (toolCalls["length"] > 0))) {
            ...
            try
            {
                ...
                if (AND((AND((argsObj), (type_of(argsObj) == std::string("object")))), (Object->keys(argsObj)->get_length() > 0))) {
                    if (argsObj["command"]) {
                        ...
                        {
                            return AND((AND((v != std::nullopt), (v != nullptr))), (v != string_empty));
                        }
                        );
        ...
        auto thought = (type_of(modelResponse) != std::string("string")) ? any(OR((modelResponse->message), (string_empty))) (string_empty);
