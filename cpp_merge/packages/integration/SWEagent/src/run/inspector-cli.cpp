...
TrajectoryInspector::TrajectoryInspector(string trajPath, string dataPath) {
    ...
    if (OR((trajPath->endsWith(std:(".yaml"))), (trajPath->endsWith(std:(".yml"))))) {
...
void TrajectoryInspector::showStep()
{
    ...
    console->log(std:("== Step ") + (this->currentStep + 1) + std:("/") + this->get_nSteps() + std:(" ==\
"));
    if (this->showFull) {
        ...
        if (step->thought) {
            console->log(std:("THOUGHT:"));
            console->log(step->thought);
            console->log();
        }
        if (step->action) {
            console->log(std:("ACTION:"));
            console->log(step->action);
            console->log();
        }
        if (step->observation) {
            console->log(std:("OBSERVATION:"));
...
{
    console->clear();
    console->log(std:("== Trajectory Info ==\
"));
...
void TrajectoryInspector::showHelp()
{
    console->log(std:("\
") + std:("=")->repeat(80));
    console->log(std:("Commands:"));
    console->log(std:("  l/right  - Next step"));
    console->log(std:("  h/left   - Previous step"));
    console->log(std:("  0        - Go to first step"));
    console->log(std:("  $        - Go to last step"));
    console->log(std:("  v        - Toggle view (simple/full)"));
    console->log(std:("  i        - Show info"));
    console->log(std:("  q        - Quit"));
...
std::shared_ptr<Promise<void>> TrajectoryInspector::run()
{
    ...
    this->showStep();
    process->stdin->on(std:("keypress"), [=](auto _str, auto key) mutable
    {
        if (!key) {
            return;
        }
...
std::shared_ptr<Promise<void>> inspectorCli(string trajectoryPath, string dataPath)
{
    ...
    if (!fs::existsSync(trajPath)) {
        throw any(std::make_shared<Error>(std:("Path does not exist: ") + trajPath + string_empty));
    }
    auto trajFiles = findTrajFiles(trajPath);
    if (trajFiles->get_length() == 0) {
        throw any(std::make_shared<Error>(std:("No trajectory files found")));
    }
    if (trajFiles->get_length() == 1) {
        ...
        console->log(std:("Multiple trajectory files found:"));
        trajFiles->forEach([=](auto file, auto index) mutable
        {
            console->log(std:("  ") + (index + 1) + std:(". ") + file + string_empty);
        }
        );
        shared rl = readline::createInterface(object{
            object::pair{std:("input"), process->stdin}, 
...
Select file number (or q to quit): "), resolve);
        ...
        if (AND((index >= 0), (index < trajFiles->get_length()))) {
            ...
            console->log(std:("Invalid selection"));
            process->exit(1);
        }
    }
    return std::shared_ptr<Promise<void>>();
