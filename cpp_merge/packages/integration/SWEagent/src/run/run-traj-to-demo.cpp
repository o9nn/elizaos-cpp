...
void saveDemo(any data, string file, string trajPath)
{
    ...
    auto header = DEMO_COMMENT->replace(std:("{traj_path}"), trajPath);
    fs::writeFileSync(file, string_empty + header + std:("\
...
{
    auto traj = JSON->parse(fs::readFileSync(trajPath, std:("utf-8")));
    auto replayConfig = traj["replay_config"];
    if (type_of(traj["replay_config"]) == std:("string")) {
        replayConfig = JSON->parse(traj["replay_config"]);
    }
    auto history = OR((traj["history"]), (array<any>()));
    shared copyFields = std::make_shared<Set>(array<string>{ std:("content"), std:("role"), std:("tool_calls"), std:("agent"), std:("message_type"), std:("tool_call_ids") });
    shared admissibleRoles = (includeUser) ? std::make_shared<Set>(array<string>{ std:("assistant"), std:("user"), std:("tool") }) : std::make_shared<Set>(array<string>{ std:("assistant") });
    auto filteredHistory = history["filter"]([=](auto step) mutable
    {
        return AND((AND((admissibleRoles->has(as<string>(step->role))), ((OR((OR((!step->agent), (step->agent == std:("main")))), (step->agent == std:("primary"))))))), (!step->is_demo));
    }
    ...
    auto outputData = object{
        object::pair{std:("history"), filteredHistory}, 
        object::pair{std:("replay_config"), replayConfig}
    };
    saveDemo(outputData, outputFile, trajPath);
    logger->info(std:("Saved demo to ") + outputFile + string_empty);
};
...
void trajToDemo(string trajPath, string outputDir, string suffix, boolean overwrite, boolean includeUser)
{
    ...
    auto trajName = path->basename(trajPath, std:(".traj"));
    auto outputSubdir = path->basename(trajDir) + suffix;
    auto outputFile = path->join(outputDir, outputSubdir, string_empty + trajName + std:(".demo.yaml"));
    if (AND((fs::existsSync(outputFile)), (!overwrite))) {
        throw any(std::make_shared<Error>(std:("Output file already exists: ") + outputFile + std:(". Use --overwrite to overwrite.")));
    }
...
std::shared_ptr<AgentLogger> logger = getLogger(std:("traj2demo"));
string DEMO_COMMENT = std:("# This is a demo file generated from trajectory file:\
# {traj_path}\
