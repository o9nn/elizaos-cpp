...
void mergePredictions(array<string> directories, string output)
{
    ...
    for (auto& pred : preds)
    {
        ...
        predData["model_patch"] = (AND((predData["model_patch"] != nullptr), (predData["model_patch"] != std::nullopt))) ? any(String(predData["model_patch"])) (string_empty);
