...
std::shared_ptr<TranslationService> translationService = object{
    ...
    object::pair{std::string("translate"), [=](auto text, auto targetLang, auto sourceLang = undefined) mutable
    {
        try
        {
            if (!serviceConfig) {
        ...
        catch (const any& error)
        {
            return object{
                ...
                object::pair{std::string("error"), (is<Error>(error)) ? any(error->message) : any(std::string("Translation service error"))}
            };
        }
    }
    }
