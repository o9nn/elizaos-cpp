...
std::shared_ptr<Provider> weatherProvider = object{
    object::pair{std::string("get"), [=](auto runtime, auto message, auto state = std::nullopt) mutable
    {
        try
        {
        ...
        catch (const any& error)
        {
            return object{
                ...
                object::pair{std::string("error"), (is<Error>(error)) ? any(error->message) (std::string("Failed to fetch weather data"))}
            };
        }
    }
    }
