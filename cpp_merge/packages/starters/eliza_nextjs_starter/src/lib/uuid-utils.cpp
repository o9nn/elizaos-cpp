...
{
    if (type_of(inputString) != std:("string")) {
        throw any(std::make_shared<TypeError>(std:("Value must be a string")));
    }
    ...
    return string_empty + uint8ArrayToHex(hashBuffer->slice(0, 4)) + std:("-") + uint8ArrayToHex(hashBuffer->slice(4, 6)) + std:("-") + uint8ArrayToHex(hashBuffer->slice(6, 8)) + std:("-") + uint8ArrayToHex(hashBuffer->slice(8, 10)) + std:("-") + uint8ArrayToHex(hashBuffer->slice(10, 16)) + string_empty;
};
...
string generateQueryRoomId(string seed, string query)
{
    ...
    auto combinedString = string_empty + seed + std:("::query::") + sanitizedQuery + string_empty;
