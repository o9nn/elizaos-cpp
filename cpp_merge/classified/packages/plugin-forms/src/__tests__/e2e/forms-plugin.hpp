#ifndef _HOME_RUNNER_WORK_ELIZAOS_CPP_ELIZAOS_CPP_CLASSIFIED_PACKAGES_PLUGIN_FORMS_SRC___TESTS___E2E_FORMS_PLUGIN_TEST_H
#define _HOME_RUNNER_WORK_ELIZAOS_CPP_ELIZAOS_CPP_CLASSIFIED_PACKAGES_PLUGIN_FORMS_SRC___TESTS___E2E_FORMS_PLUGIN_TEST_H
#include "core.hpp"
#include <string>
        ...
        object::pair{std::string("fn"), [=](auto runtime) mutable
        {
            ...
            console->log(std::string("Updated form fields:"), const_(updatedForm->steps)[0]->fields->map([=](auto f) mutable
            {
                return (object{
                    object::pair{std::string("id"), f->id}, 
                    object::pair{std::string("value"), f->value}, 
                    object::pair{std::string("hasValue"), f->value != std::nullopt}
                });
            }
            ));
            auto completedForms = std::async([=]() { formsService->listForms(std::string("completed")); });
