...
std::shared_ptr<Action> createFormAction = object{
    ...
    object::pair{std::string("handler"), [=](auto runtime, auto message, auto _state = std::nullopt, auto options = std::nullopt, auto callback = std::nullopt) mutable
    {
        auto formsService = runtime->getService<std::shared_ptr<FormsService>>(std::string("forms"));
        if (!formsService) {
...
Please provide the following information:");
                ...
                {
                    responseText += std::string("\
- ") + field["label"] + string_empty + (field["description"]) ? any(std::string(": ") + field["description"] + string_empty) (string_empty) + string_empty;
                }
                );
            }
        }
