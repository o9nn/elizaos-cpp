...
std::function<object()> useGithubReleases = [=]() mutable
{
        ...
        assets->forEach([=](auto asset) mutable
        {
            ...
            if (filename->includes(std::string(".dmg"))) {
                links->push(object{
                    ...
                    object::pair{std::string("architecture"), (filename->includes(std::string("universal"))) ? any(std::string("universal")) ((filename->includes(std::string("arm64"))) ? std::string("arm64") : std::string("x64"))}, 
    ...
    shared fetchReleases = [=]() mutable
    {
        ...
        setError(nullptr);
        {
            ...
            {
                setError((is<Error>(err)) ? any(err->message) (std::string("Failed to fetch releases")));
            }
        }
    };
