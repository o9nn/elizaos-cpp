#ifndef _HOME_RUNNER_WORK_ELIZAOS-CPP_ELIZAOS-CPP_CLASSIFIED_PACKAGES_PLUGIN-SQL_SRC_PG_ADAPTER_H
#define _HOME_RUNNER_WORK_ELIZAOS-CPP_ELIZAOS-CPP_CLASSIFIED_PACKAGES_PLUGIN-SQL_SRC_PG_ADAPTER_H
#include "core.h"
...
public:
    ...
    virtual std::shared_ptr<Promise<array<std::shared_ptr<Entity>>>> getEntitiesByIds(array<std::shared_ptr<UUID>> entityIds);
...
std::shared_ptr<Promise<T>> PgDatabaseAdapter::withDatabase(std::function<std::shared_ptr<Promise<T>>()> operation)
{
    return std::async([=]() { this->withRetry([=]() mutable
    {
        ...
        {
            utils::finally __finally1954_1990([&]() mutable
            {
                client->release();
            });
            try
