...
");
            ...
            {
                auto tags = (AND((goal->tags), (goal->tags->get_length() > 0))) ? any(std::string(" [") + goal->tags->join(std::string(", ")) + std::string("]")) (string_empty);
...
");
            ...
            {
                auto completedDate = (goal->completedAt) ? any(((std::make_shared<Date>(goal->completedAt)))->toLocaleDateString()) (std::string("Unknown date"));
