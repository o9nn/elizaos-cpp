...
") + (pendingTask->priority) ? any(std::string("Priority: ") + pendingTask->priority + string_empty) (string_empty) + std::string("\
") + (pendingTask->urgent) ? std::string("Urgent: Yes") : string_empty + std::string("\
") + (pendingTask->dueDate) ? any(std::string("Due Date: ") + pendingTask->dueDate + string_empty) (string_empty) + std::string("\
") + (pendingTask->recurring) ? any(std::string("Recurring: ") + pendingTask->recurring + string_empty) (string_empty) + std::string("\
");
        ...
        return object{
            ...
            object::pair{std::string("modifications"), (parsedResult->modifications == std::string("none")) ? any(std::nullopt) (parsedResult->modifications)}
        };
    }
    ...
    object::pair{std::string("handler"), [=](auto runtime, auto message, auto state, auto options, auto callback = std::nullopt) mutable
    {
        try
        {
            ...
            if (pendingGoal->taskType == std::string("daily")) {
                ...
                auto dueDateText = (pendingGoal->dueDate) ? any(std::string(", Due: ") + ((std::make_shared<Date>(pendingGoal->dueDate)))->toLocaleDateString() + string_empty) (string_empty);
        ...
        catch (const any& error)
        {
            ...
            return object{
                ...
                object::pair{std::string("data"), object{
                    ...
                    object::pair{std::string("errorDetails"), (is<Error>(error)) ? any(error->message) (std::string("Unknown error"))}
                }}, 
                object::pair{std::string("values"), object{
                    object::pair{std::string("success"), false}, 
