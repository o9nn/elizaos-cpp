...
");
        ...
        if (finalUpdate->urgent != std::nullopt) {
...
std::shared_ptr<Promise<std::shared_ptr<TodoData>>> applyTaskUpdate(std::shared_ptr<TodoService> dataService, std::shared_ptr<TodoData> task, std::shared_ptr<TaskUpdate> update)
{
    ...
    } : object{}), ((update->description != std::nullopt) ? object{
        object::pair{std::string("description"), update->description}
    } : object{}), ((AND((update->priority != std::nullopt), (task->type == std::string("one-off")))) ? object{
        object::pair{std::string("priority"), update->priority}
    } : object{}), ((AND((update->urgent != std::nullopt), (task->type == std::string("one-off")))) ? object{
        object::pair{std::string("isUrgent"), update->urgent}
    } : object{}), ((update->dueDate != std::nullopt) ? object{
        object::pair{std::string("dueDate"), (update->dueDate) ? any(std::make_shared<Date>(update->dueDate)) (std::nullopt)}
    } : object{}));
...
std::shared_ptr<Action> updateTodoAction = object{
    ...
    object::pair{std::string("handler"), [=](auto runtime, auto message, auto state, auto options = std::nullopt, auto callback = std::nullopt) mutable
    {
        try
        {
            if (!state) {
