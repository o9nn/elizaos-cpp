...
void Main(void)
{
    describe(std::string("InternalMessageBus EventEmitter Compatibility"), [=]() mutable
    {
        ...
        describe(std::string("emit() method"), [=]() mutable
        {
            ...
            it(std::string("should handle std::nullopt data"), [=](auto done) mutable
            {
                internalMessageBus->on(std::string("std::nullopt-event"), [=](auto data) mutable
                {
                    expect(data)->toBeNull();
                    done();
                }
                );
                internalMessageBus->emit(std::string("std::nullopt-event"), std::nullopt);
            }
            );
        ...
        ")}
            ...
            it(std::string("should handle complex data types"), [=](auto done) mutable
            {
                shared complexData = object{
                    ...
                    object::pair{std::string("undefinedValue"), std::nullopt}
                };
