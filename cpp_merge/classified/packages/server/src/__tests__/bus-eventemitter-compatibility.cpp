#include "/home/runner/work/elizaos-cpp/elizaos-cpp/classified/packages/server/src/__tests__/bus-eventemitter-compatibility.test.h"
...
void Main(void)
{
    describe(std::string("InternalMessageBus EventEmitter Compatibility"), [=]() mutable
    {
        ...
        describe(std::string("EventEmitter-like behavior"), [=]() mutable
        {
          ...
          target.addEventListener('test', () => std::cout << '1' << std::endl);\
          target.addEventListener('test', [&]() { throw new Error('boom'); });\
          target.addEventListener('test', () => std::cout << '3' << std::endl);\
          \
