...
void Main(void)
{
    describe(std::string("Basic Server Functionality"), [=]() mutable
    {
        ...
        describe(std::string("UI Enable/Disable Logic"), [=]() mutable
        {
            ...
            {
                expect(testUIEnabled(std::string("development"), std::nullopt))->toBe(true);
                expect(testUIEnabled(std::string("test"), std::nullopt))->toBe(true);
                expect(testUIEnabled(std::nullopt, std::nullopt))->toBe(true);
            }
            );
            it(std::string("should disable UI by default in production"), [=]() mutable
            {
                expect(testUIEnabled(std::string("production"), std::nullopt))->toBe(false);
            }
            );
            ...
            it(std::string("should treat empty strings as std::nullopt"), [=]() mutable
            {
