    ...
    return std::make_shared<Promise>([=](auto resolve, auto reject) mutable
    {
        ...
        childProcess->on(std::string("exit"), [=](auto code, auto signal) mutable
        {
            if (code != nullptr) {
                ...
                auto exitCode = (signal == std::string("SIGINT")) ? any(130) ((signal == std::string("SIGTERM")) ? 143 : 1);
...
{
    auto testAndCiIndicators = array<boolean>{ process->env->NODE_ENV == std::string("test"), process->env->ELIZA_TEST_MODE == std::string("true"), process->env->ELIZA_TEST_MODE == std::string("1"), process->env->ELIZA_CLI_TEST_MODE == std::string("true"), process->env->ELIZA_SKIP_LOCAL_CLI_DELEGATION == std::string("true"), process->env->ELIZA_DISABLE_LOCAL_CLI_DELEGATION == std::string("true"), process->env->BUN_TEST == std::string("true"), process->env->VITEST == std::string("true"), process->env->JEST_WORKER_ID != std::nullopt, process->argv->includes(std::string("--test")), process->argv->includes(std::string("test")), const_(process->argv)[1]->includes(std::string("test")) == true, process->env->npm_lifecycle_event == std::string("test"), process->env->CI == std::string("true"), process->env->CONTINUOUS_INTEGRATION == std::string("true"), process->env->GITHUB_ACTIONS == std::string("true"), process->env->GITLAB_CI == std::string("true"), process->env->JENKINS_URL != std::nullopt, process->env->TRAVIS == std::string("true"), process->env->CIRCLECI == std::string("true"), process->env->BUILDKITE == std::string("true"), process->env->DRONE == std::string("true"), process->env->TEAMCITY_VERSION != std::nullopt, process->env->APPVEYOR == std::string("true"), process->env->CODEBUILD_BUILD_ID != std::nullopt };
