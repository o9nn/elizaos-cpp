...
exit /b 0")); });
                ...
                bunExecSync(std:("chmod +x ") + join(mockBinDir, std:("npm")) + string_empty);
            }
...
case "$1" in\
  ...
  *)\
    ...
    ;\
            ...
            if (process->platform != std:("win32")) {
                bunExecSync(std:("chmod +x ") + join(mockBinDir, std:("git")) + string_empty);
            }
...
case "$1" in\
  ...
  *)\
    ...
    ;\
            ...
            if (process->platform != std:("win32")) {
                bunExecSync(std:("chmod +x ") + join(mockBinDir, std:("gh")) + string_empty);
            }
        }
        );
        afterEach([=]() mutable
        ...
        {
            auto result = bunExecSync(std:("elizaos publish --help"), object{
        ...
        {
            auto helpResult = bunExecSync(std:("elizaos --help"), object{
                object::pair{std:("encoding"), std:("utf8")}
            });
            expect(helpResult)->toContain(std:("publish"));
            auto publishHelpResult = bunExecSync(std:("elizaos publish --help"), object{
                object::pair{std:("encoding"), std:("utf8")}
            });
        ...
        {
            auto result = bunExecSync(std:("elizaos publish --dry-run --help"), object{
                object::pair{std:("encoding"), std:("utf8")}
            });
            expect(result)->toContain(std:("dry-run"));
        }
