...
void Main(void)
{
    ...
    Cypress->Commands->add(std::string("sendMessage"), [=](P0 contentOrMessage, auto roomId = std::nullopt) mutable
    {
    ...
    Cypress->Commands->add(std::string("uploadKnowledgeFile"), [=](auto fileName, auto content, auto fileType = std::nullopt) mutable
    {
        return cy->wrap(knowledgeHelper->uploadFile(fileName, content, OR((fileType), (std::string("text/plain")))));
    }
    );
    Cypress->Commands->add(std::string("searchKnowledge"), [=](auto query, auto count = std::nullopt) mutable
    {
    ...
    Cypress->Commands->add(std::string("waitForDocumentProcessing"), [=](auto documentId, auto timeout = std::nullopt) mutable
    {
    ...
    Cypress->Commands->add(std::string("authenticateDb"), [=](auto username = std::nullopt, auto password = std::nullopt) mutable
    {
    ...
    Cypress->Commands->add(std::string("cleanupDbTestRecords"), [=](auto tableName, auto searchPattern = std::nullopt) mutable
    {
