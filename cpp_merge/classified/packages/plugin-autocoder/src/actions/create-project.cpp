...
std::shared_ptr<Action> createProjectAction = object{
    ...
    object::pair{std::string("handler"), [=](auto runtime, auto message, auto state = std::nullopt, auto _options = std::nullopt, auto callback = std::nullopt, auto _responses = std::nullopt) mutable
    {
