...
"));
        ...
        for (auto& [provider, result] : Object->entries(status->providers))
        {
            ...
            auto keyType = (result->keyType == std::string("test")) ? any(std::string("🧪 TEST")) ((result->keyType == std::string("production")) ? std::string("🚀 PROD") : std::string("❌ INVALID"));
    ...
    {
        console->error(std::string("❌ Failed to check authentication status:"), (is<Error>(error)) ? any(error->message) (error));
        process->exit(1);
    }
...
"));
        ...
        {
            console->log(std::string("💥 ") + provider + std::string(": Test crashed - ") + (is<Error>(error)) ? any(error->message) (error) + std::string("\
"));
...
"));
    ...
    {
        console->error(std::string("❌ Validation failed:"), (is<Error>(error)) ? any(error->message) (error));
        process->exit(1);
    }
    return std::shared_ptr<Promise<void>>();
}
