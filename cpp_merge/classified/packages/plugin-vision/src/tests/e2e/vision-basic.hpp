#ifndef _HOME_RUNNER_WORK_ELIZAOS_CPP_ELIZAOS_CPP_CLASSIFIED_PACKAGES_PLUGIN_VISION_SRC_TESTS_E2E_VISION_BASIC_H
#define _HOME_RUNNER_WORK_ELIZAOS_CPP_ELIZAOS_CPP_CLASSIFIED_PACKAGES_PLUGIN_VISION_SRC_TESTS_E2E_VISION_BASIC_H
#include "core.hpp"
#include <string>
...
public:
    ...
    array<object> tests = array<object>{ object{
        ...
        object::pair{std::string("fn"), [=](auto runtime) mutable
        {
            ...
            if (state->values->visionAvailable == std::nullopt) {
        ...
        object::pair{std::string("fn"), [=](auto runtime) mutable
        {
            ...
            if (updatedScene->changePercentage != std::nullopt) {
        ...
        object::pair{std::string("fn"), [=](auto runtime) mutable
        {
            ...
            if (scene->objects->length > 0) {
                ...
                {
                    if (OR((OR((OR((!obj->id), (!obj->type))), (obj->confidence == std::nullopt))), (!obj->boundingBox))) {
            ...
            if (scene->people->length > 0) {
                ...
                for (auto& person : scene->people)
                {
                    ...
                    if (OR((OR((OR((OR((!person->id), (!person->pose))), (!person->facing))), (person->confidence == std::nullopt))), (!person->boundingBox))) {
