...
string getContextualizationPrompt(string docContent, string chunkContent, double minTokens, double maxTokens, string promptTemplate)
{
    if (OR((!docContent), (!chunkContent))) {
        console->warn(std:("Document content or chunk content is missing for contextualization."));
        return std:("Error: Document or chunk content missing.");
    }
    ...
    return promptTemplate->replace(std:("{doc_content}"), docContent)->replace(std:("{chunk_content}"), chunkContent)->replace(std:("{min_tokens}"), minTokens->toString())->replace(std:("{max_tokens}"), maxTokens->toString());
};
...
{
    if (!chunkContent) {
        console->warn(std:("Chunk content is missing for contextualization."));
        return object{
            object::pair{std:("prompt"), std:("Error: Chunk content missing.")}, 
            object::pair{std:("systemPrompt"), SYSTEM_PROMPTS["DEFAULT"]}
        };
    }
    auto chunkTokens = Math->ceil(chunkContent->get_length() / DEFAULT_CHARS_PER_TOKEN);
    if (chunkTokens > maxTokens * 0.7) {
    ...
    if (contentType) {
        if (OR((OR((OR((OR((OR((contentType->includes(std:("javascript"))), (contentType->includes(std:("typescript"))))), (contentType->includes(std:("python"))))), (contentType->includes(std:("java"))))), (contentType->includes(std:("c++"))))), (contentType->includes(std:("code"))))) {
            promptTemplate = CACHED_CODE_CHUNK_PROMPT_TEMPLATE;
            systemPrompt = SYSTEM_PROMPTS["CODE"];
        } else if (contentType->includes(std:("pdf"))) {
        ...
        } else if (OR((OR((contentType->includes(std:("markdown"))), (contentType->includes(std:("text/html"))))), (isTechnicalDocumentation(chunkContent)))) {
            promptTemplate = CACHED_TECHNICAL_PROMPT_TEMPLATE;
            systemPrompt = SYSTEM_PROMPTS["TECHNICAL"];
        }
    }
    auto formattedPrompt = promptTemplate->replace(std:("{chunk_content}"), chunkContent)->replace(std:("{min_tokens}"), minTokens->toString())->replace(std:("{max_tokens}"), maxTokens->toString());
    return object{
        object::pair{std:("prompt"), formattedPrompt}, 
        object::pair{std:("systemPrompt"), std:("systemPrompt")}
    };
};
...
string getPromptForMimeType(string mimeType, string docContent, string chunkContent)
{
    ...
    if (mimeType->includes(std:("pdf"))) {
        if (containsMathematicalContent(docContent)) {
            ...
            console->debug(std:("Using mathematical PDF prompt template"));
        } else {
            minTokens = CONTEXT_TARGETS["PDF"]["MIN_TOKENS"];
            maxTokens = CONTEXT_TARGETS["PDF"]["MAX_TOKENS"];
            console->debug(std:("Using standard PDF settings"));
        }
    } else if (OR((OR((OR((OR((OR((mimeType->includes(std:("javascript"))), (mimeType->includes(std:("typescript"))))), (mimeType->includes(std:("python"))))), (mimeType->includes(std:("java"))))), (mimeType->includes(std:("c++"))))), (mimeType->includes(std:("code"))))) {
        minTokens = CONTEXT_TARGETS["CODE"]["MIN_TOKENS"];
        maxTokens = CONTEXT_TARGETS["CODE"]["MAX_TOKENS"];
        promptTemplate = CODE_PROMPT_TEMPLATE;
        console->debug(std:("Using code prompt template"));
    } else if (OR((OR((isTechnicalDocumentation(docContent)), (mimeType->includes(std:("markdown"))))), (mimeType->includes(std:("text/html"))))) {
...
object getCachingPromptForMimeType(string mimeType, string chunkContent)
{
    ...
    if (mimeType->includes(std:("pdf"))) {
        ...
        minTokens = CONTEXT_TARGETS["CODE"]["MIN_TOKENS"];
        maxTokens = CONTEXT_TARGETS["CODE"]["MAX_TOKENS"];
    } else if (OR((OR((isTechnicalDocumentation(chunkContent)), (mimeType->includes(std:("markdown"))))), (mimeType->includes(std:("text/html"))))) {
        minTokens = CONTEXT_TARGETS["TECHNICAL"]["MIN_TOKENS"];
        maxTokens = CONTEXT_TARGETS["TECHNICAL"]["MAX_TOKENS"];
    }
...
{
    auto latexMathPatterns = array<std::shared_ptr<RegExp>>{ (new RegExp(std:("\$\$.+?\$\$"))), (new RegExp(std:("\$.+?\$"))), (new RegExp(std:("\\begin\{equation\"))), (new RegExp(std:("\\begin\{align\"))), (new RegExp(std:("\\sum"))), (new RegExp(std:("\\in"))), (new RegExp(std:("\\frac\"))), (new RegExp(std:("\\sqrt\"))), (new RegExp(std:("\\alpha|\\beta|\\gamma|\\delta|\\theta|\\lambda|\\sigm"))), (new RegExp(std:("\\nabla|\\partia"))) };
    auto generalMathPatterns = array<std::shared_ptr<RegExp>>{ (new RegExp(std:("[≠≤≥±∞∫∂∑∏√∈∉⊆⊇⊂⊃∪∩"))), (new RegExp(std:("\b[a-zA-Z]\^[0-9"))), (new RegExp(std:("\(\s*-?\d+(\.\d+)?\s*,\s*-?\d+(\.\d+)?\s*\"))), (new RegExp(std:("\b[xyz]\s*=\s*-?\d+(\.\d+)"))), (new RegExp(std:("\[\s*-?\d+(\.\d+)?\s*,\s*-?\d+(\.\d+)?\s*\"))), (new RegExp(std:("\b\d+\s*×\s*\d"))) };
    ...
    auto mathKeywords = array<string>{ std:("theorem"), std:("lemma"), std:("proof"), std:("equation"), std:("function"), std:("derivative"), std:("integral"), std:("matrix"), std:("vector"), std:("algorithm"), std:("constraint"), std:("coefficient") };
...
{
    auto technicalPatterns = array<std::shared_ptr<RegExp>>{ (new RegExp(std:("\b(version|v)\s*\d+\.\d+(\.\d+)?"))), (new RegExp(std:("\b(api|sdk|cli)\b"))), (new RegExp(std:("\b(http|https|ftp):\/\/"))), (new RegExp(std:("\b(GET|POST|PUT|DELETE)\"))), (new RegExp(std:("<\/?[a-z][\s\S]*>"))), (new RegExp(std:("\bREADME\b|\bCHANGELOG\b"))), (new RegExp(std:("\b(config|configuration)\b"))), (new RegExp(std:("\b(parameter|param|argument|arg)\b"))) };
    auto docHeadings = array<std::shared_ptr<RegExp>>{ (new RegExp(std:("\b(Introduction|Overview|Getting Started|Installation|Usage|API Reference|Troubleshooting)\b"))) };
    ...
    auto listPatterns = array<std::shared_ptr<RegExp>>{ (new RegExp(std:("\d+\.\s.+\n\d+\.\s."))), (new RegExp(std:("•\s.+\n•\s."))), (new RegExp(std:("\*\s.+\n\*\s."))), (new RegExp(std:("-\s.+\n-\s."))) };
...
string getChunkWithContext(string chunkContent, string generatedContext)
{
    if (OR((!generatedContext), (generatedContext->trim() == string_empty))) {
        console->warn(std:("Generated context is empty. Falling back to original chunk content."));
        return chunkContent;
    }
...
string CACHED_CHUNK_PROMPT_TEMPLATE = std:("\
Here is the chunk we want to situate within the whole document:\
<chunk>\
{chunk_content}\
</chunk>\
...
\
Provide ONLY the enriched chunk text in your response:");
string CACHED_CODE_CHUNK_PROMPT_TEMPLATE = std:("\
...
string CACHED_MATH_PDF_PROMPT_TEMPLATE = std:("\
Here is the chunk we want to situate within the whole document:\
<chunk>\
{chunk_content}\
</chunk>\
...
\
Provide ONLY the enriched chunk text in your response:");
string CACHED_TECHNICAL_PROMPT_TEMPLATE = std:("\
Here is the chunk we want to situate within the whole document:\
<chunk>\
{chunk_content}\
</chunk>\
...
\
Provide ONLY the enriched chunk text in your response:");
string MATH_PDF_PROMPT_TEMPLATE = std:("\
<document>\
{doc_content}\
</document>\
\
...
\
Provide ONLY the enriched chunk text in your response:");
string CODE_PROMPT_TEMPLATE = std:("\
<document>\
{doc_content}\
</document>\
\
...
\
Provide ONLY the enriched code chunk in your response:");
string TECHNICAL_PROMPT_TEMPLATE = std:("\
<document>\
{doc_content}\
</document>\
\
