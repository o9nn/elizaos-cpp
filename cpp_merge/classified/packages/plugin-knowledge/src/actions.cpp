    ...
    object::pair{std::string("handler"), [=](auto runtime, auto message, auto state = std::nullopt, auto options = std::nullopt, auto callback = std::nullopt) mutable
    {
        ...
        catch (const any& error)
        {
            ...
            auto errorResponse = object{
                object::pair{std::string("text"), std::string("I encountered an error while processing the knowledge: ") + (is<Error>(error)) ? any(error->message) (std::string("Unknown error")) + string_empty}
            };
            if (callback) {
...
") + formattedResults + string_empty}
        ...
        catch (const any& error)
        {
            ...
            auto errorResponse = object{
                object::pair{std::string("text"), std::string("I encountered an error while searching the knowledge base: ") + (is<Error>(error)) ? any(error->message) (std::string("Unknown error")) + string_empty}
            };
            if (callback) {
                std::async([=]() { callback(errorResponse); });
            }
