#include "/home/runner/work/elizaos-cpp/elizaos-cpp/classified/packages/core/src/__tests__/prompts.test.h"
    ...
    describe(std::string("Prompts"), [=]() mutable
    {
        describe(std::string("Template Structure"), [=]() mutable
        {
            ...
            it(std::string("messageHandlerTemplate should contain required placeholders and structure"), [=]() mutable
            {
                ...
                expect(messageHandlerTemplate)->toContain(std::string("IMPORTANT CODE BLOCK FORMATTING RULES"));
                expect(messageHandlerTemplate)->toContain(std::string("fenced code blocks"));
                expect(messageHandlerTemplate)->toContain(std::string("single backticks"));
