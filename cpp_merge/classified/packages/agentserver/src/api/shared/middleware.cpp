#include "/home/runner/work/elizaos-cpp/elizaos-cpp/classified/packages/agentserver/src/api/shared/middleware.h"
...
std::function<any()> createApiRateLimit = [=]() mutable
{
    return rateLimit(object{
        ...
        {
            auto exemptPaths = array<string>{ std::string("/api/server/health"), std::string("/api/ping"), std::string("/api/agents/primary"), std::string("/api/system/health"), std::string("/api/system/environment") };
            auto isExempt = exemptPaths->some([=](auto path) mutable
            {
                return req["path"]["startsWith"](path);
            }
            );
            auto isLocalhost = OR((OR((req["ip"] == std::string("127.0.0.1")), (req["ip"] == std::string("::1")))), (req["ip"] == std::string("::ffff:127.0.0.1")));
            return OR((isExempt), ((AND((isLocalhost), (process->env->NODE_ENV != std::string("production"))))));
        }
        }, 
