...
std::shared_ptr<express::Router> createLoggingRouter()
{
    ...
    auto logsHandler = [=](auto req, auto res) mutable
    {
        ...
        try
        {
            ...
            shared requestedLevelValue = (requestedLevel == std::string("all")) ? any(0) (OR((const_(LOG_LEVELS)[as<any>(requestedLevel)]), (LOG_LEVELS["info"])));
            ...
            auto filtered = recentLogs->filter([=](auto log) mutable
            {
                ...
                if (AND((requestedLevel), (requestedLevel != std::string("all")))) {
                    auto logLevel = (type_of(log->level) == std::string("number")) ? any(log->level) (OR((const_(LOG_LEVELS)[as<any>(log->level)]), (30)));
            ...
            logger->debug(std::string("Logs request processed"), object{
                ...
                object::pair{std::string("agentNamePopulationRate"), string_empty + Math->round(agentNamePopulationRate * 100) + std::string("%")}, 
                object::pair{std::string("agentIdPopulationRate"), string_empty + Math->round(agentIdPopulationRate * 100) + std::string("%")}, 
        ...
        catch (const any& error)
        {
            res->status(500)->json(object{
                ...
                object::pair{std::string("message"), (is<Error>(error)) ? any(error->message) (std::string("Unknown error"))}
            });
        }
    };
...
object LOG_LEVELS = as<std::shared_ptr<const>>(object{
    ...
    object::pair{std::string("verbose"), 0}
});
