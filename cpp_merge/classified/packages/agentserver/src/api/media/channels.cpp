#include "/home/runner/work/elizaos-cpp/elizaos-cpp/classified/packages/agentserver/src/api/media/channels.h"

std::shared_ptr<Promise<object>> saveUploadedFile(std::shared_ptr<Express::Multer::File> file, string channelId)
{
    auto uploadDir = path->join(process->cwd(), std:(".eliza/data/uploads/channels"), channelId);
