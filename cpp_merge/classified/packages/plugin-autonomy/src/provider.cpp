...
[/ADMIN_CHAT_HISTORY]")}, 
        ...
        )->slice(-10)->map([=](auto msg) mutable
        {
            ...
            auto sender = (isFromAdmin) ? any(std::string("Admin")) ((isFromAgent) ? std::string("Agent") : std::string("Other"));
        ...
        auto adminMoodContext = (recentAdminMessages->length > 0) ? any(std::string("Last admin message: "") + (OR((lastAdminMessage->content->text), (std::string("N/A")))) + std::string(""")) (std::string("No recent admin messages"));
        return object{
            object::pair{std::string("text"), std::string("[ADMIN_CHAT_HISTORY]\
