...
std::shared_ptr<Provider> autonomyStatusProvider = object{
    ...
    object::pair{std::string("get"), [=](auto runtime, auto message, auto _state = std::nullopt) mutable
    {
        ...
        auto intervalUnit = (intervalText < 60) ? any(std::string("seconds")) (string_empty + Math->round(intervalText / 60) + std::string(" minutes"));
        return object{
...
[/AUTONOMY_STATUS]")}, 
            object::pair{std::string("data"), object{
                ...
                object::pair{std::string("status"), (serviceRunning) ? any(std::string("running")) ((autonomyEnabled) ? std::string("enabled") : std::string("disabled"))}
            }}
        };
    }
    }
