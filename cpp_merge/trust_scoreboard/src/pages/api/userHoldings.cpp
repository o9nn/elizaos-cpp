...
any handler(std::shared_ptr<NextApiRequest> req, std::shared_ptr<NextApiResponse> res)
{
    ...
    try
    {
        ...
        auto holdings = data["result"]["items"]["map"]([=](auto item) mutable
        {
            ...
            auto decimals = (tokenInfo["decimals"] != std::nullopt) ? any(tokenInfo["decimals"]) (9);
    ...
    catch (const any& error)
    {
        ...
        return res->status(500)->json(object{
            object::pair{std::string("error"), (is<Error>(error)) ? any(error->message) (std::string("Failed to fetch holdings"))}
        });
    }
};
