...
std::shared_ptr<Promise<string>> sendTransaction(std::shared_ptr<Connection> connection, array<any> instructions, std::shared_ptr<Keypair> wallet)
{
    ...
    auto messageV0 = ((std::make_shared<TransactionMessage>(object{
        object::pair{std:("payerKey"), wallet->publicKey}, 
        object::pair{std:("recentBlockhash"), latestBlockhash->blockhash}, 
        object::pair{std:("instructions"), std:("instructions")}
    })))->compileToV0Message();
    ...
    auto computeBudgetInstructions = array<any>{ ComputeBudgetProgram->setComputeUnitLimit(object{
        object::pair{std:("units"), safeComputeUnits}
    }), ComputeBudgetProgram->setComputeUnitPrice(object{
        object::pair{std:("microLamports"), prioritizationFee}
    }) };
    auto finalMessage = ((std::make_shared<TransactionMessage>(object{
        object::pair{std:("payerKey"), wallet->publicKey}, 
        object::pair{std:("recentBlockhash"), latestBlockhash->blockhash}, 
        object::pair{std:("instructions"), array<any>{ computeBudgetInstructions, instructions }}
    })))->compileToV0Message();
    ...
    while (Date->now() - startTime < timeoutMs)
    {
        ...
        if (const_(statuses->value)[0]) {
            if (!const_(statuses->value)[0]->err) {
                elizaLogger->log(std:("Transaction confirmed: ") + signature + string_empty);
                return signature;
            } else {
                throw any(std::make_shared<Error>(std:("Transaction failed: ") + const_(statuses->value)[0]->err->toString() + string_empty));
            }
        }
    ...
    throw any(std::make_shared<Error>(std:("Transaction timeout")));
};
