...
std::function<any(string, double, any, any, any, double, any)> createMockRecForSvcTest = [=](auto id, auto timestamp, P2 type, auto conviction, auto userIdToSet, auto priceAtRec = std::nullopt, auto metric = std::nullopt) mutable
{
    return (object{
        object::pair{std::string("id"), asUUID(uuidv4())}, 
...
array<std::shared_ptr<TestCase>> calculateUserTrustScoreTestCases = array<std::shared_ptr<TestCase>>{ object{
    ...
    object::pair{std::string("name"), std::string("Service.calculateUserTrustScore: Recommendation with std::nullopt metrics triggers re-evaluation")}, 
    object::pair{std::string("fn"), [=](auto runtime) mutable
    {
        auto service = std::make_shared<CommunityInvestorService>(runtime);
        shared recTimestamp = Date->now();
        auto recs = array<any>{ createMockRecForSvcTest(std::string("needsEval"), recTimestamp, std::string("BUY"), Conviction->MEDIUM, testUserIdGlobalForService, 10, std::nullopt) };
        auto initialProfile = object{
            object::pair{std::string("version"), std::string("1.0.0")}, 
            object::pair{std::string("userId"), testUserIdGlobalForService}, 
            object::pair{std::string("trustScore"), 0}, 
...
array<std::shared_ptr<TestCase>> getLeaderboardDataTestCases = array<std::shared_ptr<TestCase>>{ object{
    ...
    object::pair{std::string("fn"), [=](auto runtime) mutable
    {
        ...
        (as<any>(runtime->getComponent)) = [=](auto entityId, auto type, auto worldId = std::nullopt) mutable
        {
