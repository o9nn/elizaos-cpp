...
TokenPerformance transformTokenPerformance(any dbToken, string chain)
{
    auto input = object{
        ...
        object::pair{std::string("price"), (type_of(dbToken["price"]) == std::string("number")) ? any(dbToken["price"]) (0)}, 
        object::pair{std::string("volume"), (type_of(dbToken["volume"]) == std::string("number")) ? any(dbToken["volume"]) (0)}, 
        object::pair{std::string("trades"), (type_of(dbToken["trades"]) == std::string("number")) ? any(dbToken["trades"]) (0)}, 
        object::pair{std::string("liquidity"), (type_of(dbToken["liquidity"]) == std::string("number")) ? any(dbToken["liquidity"]) (0)}, 
        object::pair{std::string("holders"), (type_of(dbToken["holders"]) == std::string("number")) ? any(dbToken["holders"]) (0)}, 
        object::pair{std::string("price24hChange"), (type_of(dbToken["price_change_24h"]) == std::string("number")) ? any(dbToken["price_change_24h"]) (0)}, 
        object::pair{std::string("volume24hChange"), (type_of(dbToken["volume_change_24h"]) == std::string("number")) ? any(dbToken["volume_change_24h"]) (0)}, 
        object::pair{std::string("trades24hChange"), (type_of(dbToken["trade_24h_change"]) == std::string("number")) ? any(dbToken["trade_24h_change"]) (0)}, 
        object::pair{std::string("holders24hChange"), (type_of(dbToken["holder_change_24h"]) == std::string("number")) ? any(dbToken["holder_change_24h"]) (0)}, 
        object::pair{std::string("initialMarketCap"), (type_of(dbToken["initial_market_cap"]) == std::string("number")) ? any(dbToken["initial_market_cap"]) (0)}, 
        object::pair{std::string("currentMarketCap"), (type_of(dbToken["current_market_cap"]) == std::string("number")) ? any(dbToken["current_market_cap"]) (0)}, 
        ...
        object::pair{std::string("validationTrust"), (type_of(dbToken["validation_trust"]) == std::string("number")) ? any(dbToken["validation_trust"]) (0)}, 
...
{
    auto type = (type_of(dbTx["type"]) == std::string("string")) ? any(dbTx["type"]["toLowerCase"]()) (TransactionType["BUY"]);
    auto input = object{
        ...
        object::pair{std::string("type"), (OR((type == std::string("BUY")), (type == std::string("SELL")))) ? any(type) (std::string("BUY"))}, 
        object::pair{std::string("amount"), (type_of(dbTx["amount"]) == std::string("bigint")) ? any(dbTx["amount"]) ((type_of(dbTx["amount"]) == std::string("string")) ? BigInt(dbTx["amount"]) : (type_of(dbTx["amount"]) == std::string("number")) ? BigInt(Math->floor(dbTx["amount"])) : BigInt(0))}, 
        object::pair{std::string("price"), (type_of(dbTx["price"]) == std::string("string")) ? any(Number(dbTx["price"])) ((type_of(dbTx["price"]) == std::string("number")) ? any(dbTx["price"]) (std::nullopt))}, 
        object::pair{std::string("isSimulation"), Boolean(OR((dbTx["isSimulation"]), (dbTx["is_simulation"])))}, 
        object::pair{std::string("timestamp"), (is<Date>(dbTx["timestamp"])) ? dbTx["timestamp"]["toISOString"]() : (type_of(dbTx["timestamp"]) == std::string("string")) ? any(dbTx["timestamp"]) (((std::make_shared<Date>()))->toISOString())}
    };
...
Position transformPosition(any dbPos)
{
    auto input = object{
        ...
        object::pair{std::string("performanceScore"), (type_of(dbPos["performanceScore"]) == std::string("number")) ? dbPos["performanceScore"] : (type_of(dbPos["performance_score"]) == std::string("number")) ? any(dbPos["performance_score"]) (0)}, 
        object::pair{std::string("rapidDump"), Boolean(OR((dbPos["rapidDump"]), (dbPos["rapid_dump"])))}, 
        object::pair{std::string("openedAt"), (is<Date>(dbPos["openedAt"])) ? dbPos["openedAt"] : (is<Date>(dbPos["opened_at"])) ? any(dbPos["opened_at"]) ((type_of(dbPos["openedAt"]) == std::string("string")) ? std::make_shared<Date>(dbPos["openedAt"]) : (type_of(dbPos["opened_at"]) == std::string("string")) ? std::make_shared<Date>(dbPos["opened_at"]) : std::make_shared<Date>())}, 
        object::pair{std::string("closedAt"), (is<Date>(dbPos["closedAt"])) ? dbPos["closedAt"] : (is<Date>(dbPos["closed_at"])) ? any(dbPos["closed_at"]) ((type_of(dbPos["closedAt"]) == std::string("string")) ? std::make_shared<Date>(dbPos["closedAt"]) : (type_of(dbPos["closed_at"]) == std::string("string")) ? any(std::make_shared<Date>(dbPos["closed_at"])) (std::nullopt))}, 
        object::pair{std::string("updatedAt"), (is<Date>(dbPos["updatedAt"])) ? dbPos["updatedAt"] : (is<Date>(dbPos["updated_at"])) ? any(dbPos["updated_at"]) ((type_of(dbPos["updatedAt"]) == std::string("string")) ? std::make_shared<Date>(dbPos["updatedAt"]) : (type_of(dbPos["updated_at"]) == std::string("string")) ? std::make_shared<Date>(dbPos["updated_at"]) : std::make_shared<Date>())}, 
