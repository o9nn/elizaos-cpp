...
std::shared_ptr<Promise<object>> executeTrade(std::shared_ptr<IAgentRuntime> runtime, object params)
{
    ...
    try
    {
        ...
        auto inputTokenCA = (params["action"] == std::string("SELL")) ? any(params["tokenAddress"]) (SOL_ADDRESS);
        auto outputTokenCA = (params["action"] == std::string("SELL")) ? any(SOL_ADDRESS) (params["tokenAddress"]);
    ...
    catch (const any& error)
    {
        logger->error(std::string("Trade execution failed:"), object{
            ...
            object::pair{std::string("errorStack"), (is<Error>(error)) ? any(error->stack) (std::nullopt)}
        });
        return object{
            object::pair{std::string("success"), false}, 
            object::pair{std::string("error"), (is<Error>(error)) ? any(error->message) (std::string("Unknown error"))}
...
std::shared_ptr<Promise<object>> executeRaydiumTrade(std::shared_ptr<IAgentRuntime> runtime, object params)
{
    try
    {
        ...
        auto quoteResponse = std::async([=]() { fetch(std::string("https://api.raydium.io/v2/main/quote"), object{
            ...
            object::pair{std::string("body"), JSON->stringify(object{
                object::pair{std::string("inputMint"), (params["isSell"]) ? any(params["tokenAddress"]) (SOL_ADDRESS)}, 
                object::pair{std::string("outputMint"), (params["isSell"]) ? any(SOL_ADDRESS) (params["tokenAddress"])}, 
                object::pair{std::string("amount"), params["amount"]}, 
