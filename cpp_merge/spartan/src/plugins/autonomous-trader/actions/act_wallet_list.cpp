...
std::shared_ptr<Action> userMetawalletList = object{
    ...
    object::pair{std::string("handler"), [=](auto runtime, auto message, auto state, auto _options, auto callback = std::nullopt, auto responses) mutable
    {
        ...
        takeItPrivate(runtime, message, std::string("List wallets: ") + JSON->stringify(email->data->metawallets));
