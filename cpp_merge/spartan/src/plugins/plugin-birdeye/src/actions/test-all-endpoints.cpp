...
any testAllEndpointsAction = as<std::shared_ptr<Action>>(object{
    ...
    object::pair{std::string("handler"), [=](auto runtime, auto _message, auto _state, auto _options, auto callback = std::nullopt) mutable
    {
        try
        {
            ...
            auto sampleParams = object{
                ...
                object::pair{std::string("sort_by"), std::nullopt}, 
            ...
            std::async([=]() { birdeyeProvider->fetchTraderTransactionsSeek(utils::assign(object{
                , 
                object::pair{std::string("tx_type"), std::string("all")}, 
                object::pair{std::string("before_time"), std::nullopt}
            }, sampleParams)); });
