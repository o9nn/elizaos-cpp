...
any tokenSearchSymbolAction = as<std::shared_ptr<Action>>(object{
    ...
    object::pair{std::string("handler"), [=](auto runtime, auto message, auto _state, auto _options, auto callback = std::nullopt) mutable
    {
        try
        {
...
");
        ...
        output += std::string("Search Result #") + (tokens->get_length() > 0) ? any(i + 1) (string_empty) + std::string(":\
");
