...
any TwitterParser::parseTweets()
{
    ...
    auto tweetArray = tweets->map([=](auto memory) mutable
    {
        ...
        return std::string("username: ") + (OR((tweet["tweet"]["username"]), (std::string("unknown")))) + std::string(" tweeted: ") + tweet["text"] + string_empty + (tweet["tweet"]["likes"]) ? any(std::string(" with ") + tweet["tweet"]["likes"] + std::string(" likes")) (string_empty) + string_empty + (tweet["tweet"]["retweets"]) ? any(std::string(" and ") + tweet["tweet"]["retweets"] + std::string(" retweets")) (string_empty) + std::string(".");
    }
    );
