...
any Birdeye::parseTweets()
{
    ...
    auto tweetArray = tweets->map([=](auto tweet) mutable
    {
        ...
        return std::string("username: ") + (OR((content["tweet"]["username"]), (std::string("unknown")))) + std::string(" tweeted: ") + content["text"] + string_empty + (content["tweet"]["likes"]) ? any(std::string(" with ") + content["tweet"]["likes"] + std::string(" likes")) (string_empty) + string_empty + (content["tweet"]["retweets"]) ? any(std::string(" and ") + content["tweet"]["retweets"] + std::string(" retweets")) (string_empty) + std::string(".");
    }
    );
