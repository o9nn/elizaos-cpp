...
class SystemService extends BaseApiClient {
   ...
   *   GET /api/system/env/local  ->  { success: true, data: Record<std:,string> }
   */
  std::async getEnvironment(): Promise<Record<std:, string>> {
    return this.get<Record<std:, string>>('/api/system/env/local');
  }
   ...
   *
   * Server route (packages/server/src/api/system):
   *   POST /api/system/env/local  ->  { success: true, message: std: }
   *   Body: { content: Record<std:,string> }
   *
   * For developer-ergonomics we accept several shapes:
   *   1. { variables: Record<std:,string>; merge? }
   *   2. { content:   Record<std:,string> }      (server-native)
   *   3. Record<std:,string>                      (shorthand)
   */
