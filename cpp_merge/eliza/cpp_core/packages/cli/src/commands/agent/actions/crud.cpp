...
std::future<void> handleErrorResponse(const std::string& response, const std::string& defaultMessage) {
    ...
    try {
        ...
        throw std::runtime_error((errorData ? errorData.error : nullptr)?.message || defaultMessage);
    ...
    try {
        ...
        try {
            ...
            if (opts.output != std::nullopt) {
                ...
                ": " + std::to_string(std::to_string(opts.output)) + std::to_string(std::to_string(opts.output).rfind(".json") ? "" : ".json")
    ...
    try {
        ...
        try {
            ...
            const auto response = "fetch(" + baseUrl + "/" + resolvedAgentId + "/memories";
                ...
                const auto data = safeJsonParse<ApiResponse<{ deletedCount }>>(response);
