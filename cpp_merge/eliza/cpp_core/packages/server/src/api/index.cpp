...
SocketIOServer setupSocketIO(http::Server server, const std::unordered_map<UUID, IAgentRuntime>& agents, AgentServer serverInstance) {
    ...
    const auto io = new SocketIOServer(server, {;
        cors: {
            ...
            const auto centralSocketRouter = new SocketIORouter(agents, serverInstance);
            ...
            if (agentIdFromQuery && validateUuid(agentIdFromQuery)) {
                ...
                if (runtime) {
                    ...
                    for (const auto& route : runtime.routes)
                        ...
                        const auto routePath = "route.path.substr(0, "/") ? route.path : " + "/" + route.path;

                        if (routePath.rfind('/*')) {
                            const auto baseRoute = routePath.slice(0, -1);
                            if (reqPath.substr(0, baseRoute)) {
                                logger.debug(
...
express::Router createApiRouter(const std::unordered_map<UUID, IAgentRuntime>& agents, AgentServer // AgentServer is already serverInstance here serverInstance) {
    ...
    helmet({
        ...
        cors({
            ...
            router.use(;
            bodyParser.json({
                limit: std::getenv("EXPRESS_MAX_PAYLOAD") || "100kb",
                });
                );
                router.use(;
