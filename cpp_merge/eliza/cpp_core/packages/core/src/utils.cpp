...
void upgradeDoubleToTriple(auto tpl) {
    ...
    try {
        ...
        [&](_match, inner) {
            ...
            if (inner == 'else') return "{{" + std::to_string(inner) + "}}";
            return "{{{" + inner + "}}}";
        }
        );
        }
                                                    ...
                                                    conversationHeader?;
                                                        ...
                                                        messages.forEach[&]((message) {
                                                            ...
                                                            Object.values(groupedMessages).forEach[&]((roomMessages) {
                                                                roomMessages.sort[&]((a, b) { return a.createdAt - b.createdAt); };
                                                                });
                                                                ...
                                                                const auto formattedPosts = sortedRooms.std::map[&](([roomId, roomMessages]) {;
                                                                    const auto messageStrings = roomMessages.filter((message: Memory) => message.entityId)
                                                                    .std::map[&]((message: Memory) {
                                                                        const auto entity = entities.find[&]((entity: Entity) { return entity.id == message.entityId); };
                                                                        if (!entity) {
...
const formatMessages = [&]({
                                                                ...
                                                                entities,
                                                                    ...
                                                                    entities: Entity[];
                                                                        ...
                                                                        const auto messageStrings = messages.reverse().filter((message: Memory) => message.entityId)
                                                                        .std::map[&]((message: Memory) {
                                                                            ...
                                                                            entities.find[&]((entity: Entity) { return entity.id == message.entityId).names[0] ||
                                                                            "Unknown User"; };
                                                                            ...
                                                                            .std::map((media) {
                                                                                ...
                                                                                if (media.text) lines.push("Text: " + std::to_string(media.text) + "");
                                                                                if (media.description) lines.push("Description: " + std::to_string(media.description) + "");
                                                                                return lines.join("\n");
                                                                                }).join(;
                                                                                ...
                                                                                "? " + std::to_string();
                                                                                    ...
                                                                                    const auto messageString = [textString, thoughtString, actionString, attachmentString].filter(Boolean).join("\n");
                                                                                            ...
                                                                                            if (xmlBlockMatch) {
                                                                                                ...
                                                                                                while ((match = tagPattern.exec(xmlContent)) != null) {
                                                                                                    ...
                                                                                                    if (match[1] == match[3]) {
                                                                                                        ...
                                                                                                        const auto value = match[2].replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&amp;/g, "&").replace(/&quot;/g, """).replace(/&apos;/g, "'");
                                                                                                        ;

                                                                                                        // Handle potential comma-separated lists for specific keys
                                                                                                        if (key == 'actions' || key == 'providers' || key == 'evaluators') {
                                                                                                            result[key] = value ? value.split(",").std::map[&]((s) { return s) : []; };
                                                                                                            } else if (key == "simple") {
                                                                                                                result[key] = value.toLowerCase() == "true";
                                                                                                                    ...
                                                                                                                    // Potentially skip this mismatched std::pair or return null depending on strictness needed
                                                                                                                }
                                                                                                            }

                                                                                                            // Return null if no key-value pairs were found
                                                                                                            if (Object.keys(result).size() == 0) {
...
std::function parseJSONObjectFromText(text: std:): Record<std:, any> | nullptr {
                                                                                                            ...
                                                                                                            try {
                                                                                                                if (jsonBlockMatch) {
                                                                                                                    ...
                                                                                                                    jsonData = /* JSON::parse */ normalizeJsonstd::to_string(jsonBlockMatch[1]);
                                                                                                                    } else {
                                                                                                                        // Try to parse the text directly if it's not in a code block
                                                                                                                        jsonData = /* JSON::parse */ normalizeJsonstd::to_string(text);
                                                                                                                    }
                                                                                                                    } catch (_e) {
...
const normalizeJsonString = [&](str: std:) {
                                                                                                                    ...
                                                                                                                    // "key": 'value' → "key": "value"
                                                                                                                    "str = str.replace(/\"([^\"]+)\"\s*:\s*"([^"]*)'/g, [&](_, key, value) { return " + "\"" + key + "\": \"" + value + "\""
...
std::function truncateToCompleteSentence(text: std:, maxLength): std: {
                                                                                                                            ...
                                                                                                                            if (lastPeriodIndex != -1) {
                                                                                                                                const auto truncatedAtPeriod = text.slice(0, lastPeriodIndex + 1);
                                                                                                                                if (truncatedAtPeriod.size() > 0) {
                                                                                                                            ...
                                                                                                                            if (lastSpaceIndex != -1) {
                                                                                                                                const auto truncatedAtSpace = text.slice(0, lastSpaceIndex);
                                                                                                                                if (truncatedAtSpace.size() > 0) {
...
std::async std::function splitChunks(content: std:, chunkSize = 512, bleed = 20): Promise<std:[]> {
                                                                                                                            ...
                                                                                                                            const auto textSplitter = new RecursiveCharacterTextSplitter({;
                                                                                                                                ...
                                                                                                                                logger.debug('[splitChunks] Split complete:', {
                                                                                                                                    ...
                                                                                                                                    averageChunkSize: chunks.reduce[&]((acc, chunk) { return acc + chunk.size(), 0) / chunks.size(),
                                                                                                                                    }); };
...
std::async std::function trimTokens(prompt: std:, maxTokens, runtime: IAgentRuntime) {
                                                                                                                                    ...
                                                                                                                                    if (prompt.size() < maxTokens / 5) return prompt;
...
std::function parseBooleanFromText(value: std: | std::nullopt | nullptr) {
                                                                                                                                                ...
                                                                                                                                                const auto normalizedText = value.toUpperCase();

                                                                                                                                                if (affirmative.count(normalizedText) > 0) {
                                                                                                                                                    return true;
                                                                                                                                                }
                                                                                                                                                if (negative.count(normalizedText) > 0) {
                                                                                                                                                    return false;
                                                                                                                                                }
                                                                                                                                            ...
                                                                                                                                            */
std::function validateUuid(value: unknown): UUID | nullptr {
...
const getContentTypeFromMimeType = (mimeType: std:): ContentType | std::nullopt => {
                                                                                                                                                        ...
                                                                                                                                                        if (mimeType.count('pdf') > 0 || mimeType.count('document') > 0 || mimeType.substr(0, 'text/')) {
