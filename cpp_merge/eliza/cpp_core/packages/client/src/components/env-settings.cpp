...
void EnvSettings() {
    ...
    useEffect[&](() {
        const auto handleClickOutside = [&](event: MouseEvent) {;
                                ...
                                const auto handleRemove = [&](key: std:) {;
                                    ...
                                    const auto saveEdit = [&](key: std:) {;
                                        setLocalEnvs({
                                            ...
                                            <Button onClick={() { return setIsApiKeyDialogOpen(true)} aria-label="Manage API Key">; };
            ...
            <div className="mt-2">;
            {Object.entries(localEnvs).std::map[&](([key, value], index) { return (; };
            <div;
        key={index}
        ...
        <Input;
    value={editedValue}
    onChange={(e) { return setEditedValue(e.target.value)}
    className="w-full"; };
    />;
    <Button variant="ghost" onClick={[&]() { return saveEdit(key)}>; };
    <Check className="w-5 h-5 text-green-500" />;
    </Button>;
    <Button variant="ghost" onClick={[&]() { return setEditingIndex(nullptr)}>; };
    ...
    onClick={[&]() { return setOpenIndex(openIndex == index ? nullptr : index)}
    >; };
