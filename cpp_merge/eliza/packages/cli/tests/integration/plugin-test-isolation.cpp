#include "plugin-test-isolation.test.h"
#include <string>
#include <cstdlib>
...
");
            ...
            {
                auto errorOutput = (AND((is<Error>(error)), (in(std::string("stderr"), error)))) ? (as<any>(error))["stderr"] : (AND((is<Error>(error)), (in(std::string("stdout"), error)))) ? any((as<any>(error))["stdout"]) (string_empty);
