...
std::shared_ptr<Action> unmuteRoomAction = as<std::shared_ptr<Action>>(object{
    ...
    object::pair{std::string("handler"), [=](auto runtime, auto message, auto state = std::nullopt, auto _options = std::nullopt, auto _callback = std::nullopt, auto _responses = std::nullopt) mutable
    {
