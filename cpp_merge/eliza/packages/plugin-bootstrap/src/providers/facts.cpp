#include "/home/runner/work/elizaos-cpp/elizaos-cpp/eliza/packages/plugin-bootstrap/src/providers/facts.h"
...
"));
            ...
            if (allFacts->get_length() == 0) {
                return object{
                    ...
                    object::pair{std::string("text"), std::string("No facts available.")}
                };
            }
...
{{formattedFacts}}")->replace(std::string("{{agentName}}"), runtime->character->name)->replace(std::string("{{formattedFacts}}"), formattedFacts);
        ...
        {
            logger->error(std::string("Error in factsProvider:"), error);
            return object{
                object::pair{std::string("values"), object{
                    object::pair{std::string("facts"), string_empty}
                }}, 
                ...
                object::pair{std::string("text"), std::string("Error retrieving facts.")}
            };
        }
    }
    }
