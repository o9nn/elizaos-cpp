...
void Main(void)
{
    describe(std::string("Agent Portability Integration Tests"), [=]() mutable
    {
        ...
        {
            it(std::string("should an agent to ZIP file"), [=]() mutable
            {
            ...
            it(std::string("should validate manifest version"), [=]() mutable
            {
                ...
                expect(response->body->error->message)->toContain(std::string("Unsupported version"));
            }
            );
        ...
        {
            it(std::string("should successfully and re-import an agent"), [=]() mutable
            {
