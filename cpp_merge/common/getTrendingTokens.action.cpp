...
std::shared_ptr<Action> getTrendingTokensAction = object{
    ...
    object::pair{std::string("handler"), [=](auto runtime, auto message, auto _state = std::nullopt, auto _options = std::nullopt, auto callback = std::nullopt) mutable
    {
        try
        {
            ...
            auto limit = (type_of(limitRaw) == std::string("number")) ? Math->max(1, Math->min(30, Math->floor(limitRaw))) : (type_of(limitRaw) == std::string("string")) ? any(Math->max(1, Math->min(30, Math->floor(OR((Number(limitRaw)), (10)))))) (10);
