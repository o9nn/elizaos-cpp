...
std::shared_ptr<Action> checkTransactionConfirmationAction = as<std::shared_ptr<Action>>(object{
    ...
    object::pair{std::string("validate"), [=](auto runtime, auto _message, auto _state = std::nullopt) mutable
    {
        try
        {
    ...
    object::pair{std::string("handler"), [=](auto runtime, auto message, auto _state, auto _options = std::nullopt, auto callback = std::nullopt) mutable
    {
        try
        {
...
Example: "0x1234567890abcdef..."");
                ...
                auto errorResult = as<any>(object{
                    ...
                    object::pair{std::string("input"), object{
                        object::pair{std::string("transactionHash"), std::nullopt}, 
                        object::pair{std::string("chain"), std::nullopt}
                    }}
                });
                if (callback) {
            ...
            auto chainName = (chain) ? any(chain->charAt(0)->toUpperCase() + chain->slice(1)) (std::string("Ethereum"));
