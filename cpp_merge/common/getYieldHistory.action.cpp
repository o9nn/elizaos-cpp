...
std::shared_ptr<Action> getYieldHistoryAction = object{
    ...
    object::pair{std::string("handler"), [=](auto runtime, auto message, auto _state = std::nullopt, auto _options = std::nullopt, auto callback = std::nullopt) mutable
    {
        try
        {
            auto svc = as<any>(runtime->getService(DefiLlamaService::serviceType));
            ...
            auto chain = OR((params["chain"]["trim"]()), (std::nullopt));
            ...
            logger->info(std::string("[GET_YIELD_HISTORY] Finding pool for: protocol=") + protocol + std::string(", token=") + token + string_empty + (chain) ? any(std::string(", chain=") + chain + string_empty) (string_empty) + string_empty);
            ...
            if (OR((!pools), (pools->get_length() == 0))) {
                auto errorMsg = std::string("No pool found for ") + protocol + std::string(" ") + token + string_empty + (chain) ? any(std::string(" on ") + chain + string_empty) (string_empty) + string_empty;
                logger->error(std::string("[GET_YIELD_HISTORY] ") + errorMsg + string_empty);
            ...
            if (OR((!chartData), (chartData->get_length() == 0))) {
                auto errorMsg = std::string("No historical data available for ") + protocol + std::string(" ") + token + string_empty + (chain) ? any(std::string(" on ") + chain + string_empty) (string_empty) + string_empty;
            ...
            auto messageText = std::string("Retrieved ") + chartData->get_length() + std::string(" days of yield history for ") + pool["project"] + std::string(" ") + pool["symbol"] + string_empty + (pool["chain"]) ? any(std::string(" on ") + pool["chain"] + string_empty) (string_empty) + string_empty;
            auto result = object{
                object::pair{std::string("pool"), object{
                    object::pair{std::string("protocol"), pool["project"]}, 
                    object::pair{std::string("token"), pool["symbol"]}, 
