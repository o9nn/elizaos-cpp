...
std::shared_ptr<Action> getYieldRatesAction = object{
    ...
    object::pair{std::string("handler"), [=](auto runtime, auto message, auto _state = std::nullopt, auto _options = std::nullopt, auto callback = std::nullopt) mutable
    {
        try
        {
            ...
            shared protocol = OR((params["protocol"]["trim"]()), (std::nullopt));
            shared token = OR((params["token"]["trim"]()), (std::nullopt));
            shared chain = OR((params["chain"]["trim"]()), (std::nullopt));
            ...
            auto searchDesc = (searchCriteria->get_length() > 0) ? any(searchCriteria->join(std::string(", "))) (std::string("all yields"));
