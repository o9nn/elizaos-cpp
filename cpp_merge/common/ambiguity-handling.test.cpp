    ...
    describe(std::string("LLM Ambiguity Handling"), [=]() mutable
    {
        ...
        describe(std::string("Bug 2: Runtime Error Fix"), [=]() mutable
        {
            it(std::string("should handle non-string actions gracefully"), [=]() mutable
            {
                auto responseContent = object{
                    object::pair{std::string("text"), std::string("Hello world")}, 
                    object::pair{std::string("actions"), array<std::nullptr_t>{ nullptr, std::nullopt, 123, std::string("IGNORE"), std::string("REPLY") }}
                };
                auto result = handleAmbiguity(responseContent);
                expect(result["actions"])->toEqual(array<std::nullptr_t>{ nullptr, std::nullopt, 123, std::string("REPLY") });
                expect(result["simple"])->toBe(false);
            }
            );
            it(std::string("should handle mixed string/non-string actions with IGNORE"), [=]() mutable
            {
                auto responseContent = object{
                    object::pair{std::string("text"), std::string("Hello world")}, 
                    object::pair{std::string("actions"), array<std::nullptr_t>{ nullptr, std::string("IGNORE"), std::nullopt, std::string("REPLY") }}
                };
                auto result = handleAmbiguity(responseContent);
                expect(result["actions"])->toEqual(array<std::nullptr_t>{ nullptr, std::nullopt, std::string("REPLY") });
                expect(result["simple"])->toBe(false);
            }
            );
        ...
        describe(std::string("Edge Cases"), [=]() mutable
        {
            ...
            it(std::string("should handle std::nullopt actions"), [=]() mutable
            {
                auto responseContent = object{
                    object::pair{std::string("text"), std::string("Hello world")}
                };
            ...
            it(std::string("should handle all non-string actions"), [=]() mutable
            {
                auto responseContent = object{
                    object::pair{std::string("text"), std::string("Hello world")}, 
                    object::pair{std::string("actions"), array<std::nullptr_t>{ nullptr, std::nullopt, 123, object{} }}
                };
                auto result = handleAmbiguity(responseContent);
                expect(result["actions"])->toEqual(array<std::nullptr_t>{ nullptr, std::nullopt, 123, object{} });
                expect(result["simple"])->toBe(false);
            }
            );
        }
