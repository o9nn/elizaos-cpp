...
std::shared_ptr<Promise<boolean>> postUpdateToDiscordChannel(std::shared_ptr<IAgentRuntime> runtime, std::shared_ptr<TeamMemberUpdate> update)
{
    try
    {
        ...
        logger->info(std::string("Retrieved report channel configs:"), object{
            ...
            object::pair{std::string("configs"), memories->map([=](auto m) mutable
            {
                return (object{
                    ...
                    object::pair{std::string("channelId"), (m["content"]->config) ? any((as<std::shared_ptr<ReportChannelConfig>>(m["content"]->config))->channelId) (std::nullopt)}
                });
            }
            )}
        });
        ...
        {
            auto serverMatch = (targetGuild) ? any(targetGuild["id"]) (std::nullopt);
            ...
            logger->info(std::string("Checking config:"), object{
                ...
                object::pair{std::string("targetGuildId"), (targetGuild) ? any(targetGuild["id"]) (std::nullopt)}, 
...
std::shared_ptr<Action> teamMemberUpdatesAction = object{
    ...
    object::pair{std::string("handler"), [=](auto runtime, auto message, auto state, auto options = object{}, auto callback = std::nullopt) mutable
    {
        try
        {
