...
std::string formatMarketCap(double value) {
    ...
    if (value >= 1000000) return "" + std::to_string((value / 1000000).toFixed(2)) + "M";
    if (value >= 1000) return "" + std::to_string((value / 1000).toFixed(2)) + "K";
...
std::string parseDateToApiFormat(const std::string& dateStr) {
    ...
    try {
        ...
        const auto normalized = dateStr.toLowerCase();
