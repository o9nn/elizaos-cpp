...
std::function<string(double, boolean, boolean)> formatNumber = [=](auto num, auto showDecimals = std::nullopt, auto hideDollarSign = std::nullopt) mutable
{
...
std::function<string(any, boolean)> fromNow = [=](P0 date, auto hideAgo = std::nullopt) mutable
{
...
std::function<any(string)> sanitizeCheckmark = [=](auto name = std::nullopt) mutable
{
