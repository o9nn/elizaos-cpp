...
std::function<std::shared_ptr<Promise<any>>(any, any, double, double, double, any, double, double, any)> swapIx = [=](auto user, auto token, auto amount, auto style, auto slippageBps = 100, auto program = std::nullopt, auto reserveToken = std::nullopt, auto reserveLamport = std::nullopt, auto configAccount = std::nullopt) mutable
{
...
std::function<std::shared_ptr<Promise<any>>(any, any, double, double, double, any, boolean)> getJupiterSwapIx = [=](auto user, auto _token, auto amount, auto style, auto slippageBps = 100, auto _connection = std::nullopt, auto isToken2022 = false) mutable
{
    auto SOL_MINT_ADDRESS = std::string("So11111111111111111111111111111111111111112");
    auto tokenMintAddress = _token->toBase58();
    auto inputMint = (style == 0) ? any(SOL_MINT_ADDRESS) (tokenMintAddress);
    auto outputMint = (style == 0) ? any(tokenMintAddress) (SOL_MINT_ADDRESS);
    auto feeAccount = std::nullopt;
    auto feePercent = 10;
