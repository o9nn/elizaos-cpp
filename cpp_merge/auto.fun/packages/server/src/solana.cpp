#include "/home/runner/work/elizaos-cpp/elizaos-cpp/auto.fun/packages/server/src/solana.h"
#include <cstdlib>

any initSolanaConfig()
{
    auto network = process->env->NETWORK;
    auto rpcUrl = getRpcUrl();
    auto umi = createUmi(rpcUrl);
    auto programId = (network == std:("devnet")) ? OR((process->env->DEVNET_PROGRAM_ID), (process->env->PROGRAM_ID)) : process->env->PROGRAM_ID;
    if (!programId) {
        throw any(std::make_shared<Error>(std:("missing program_id env var")));
    }
    any wallet;
    if (!process->env->EXECUTOR_PRIVATE_KEY) {
        throw any(std::make_shared<Error>(std:("missing EXECUTOR_PRIVATE_KEY env var")));
    }
    try
    {
        wallet = Keypair->fromSecretKey(Uint8Array->from(JSON->parse(process->env->EXECUTOR_PRIVATE_KEY)));
        console->log(std:("Created wallet from std::getenv("EXECUTOR_PRIVATE_KEY")"));
    }
    catch (const any& error)
    {
        console->error(std:("Failed to create wallet from env:"), error);
    }
    return object{
        object::pair{std:("umi"), std:("umi")}, 
        object::pair{std:("connection"), std::make_shared<Connection>(rpcUrl)}, 
        object::pair{std:("program"), nullptr}, 
        object::pair{std:("programId"), std::make_shared<PublicKey>(programId)}, 
        object::pair{std:("wallet"), std:("wallet")}, 
        object::pair{std:("network"), std:("network")}
    };
};


std::shared_ptr<Autofun> idl = JSON->parse(JSON->stringify(idlJson));
std::function<any(any, any)> getProgram = [=](auto connection, auto wallet) mutable
{
    auto provider = std::make_shared<AnchorProvider>(connection, wallet, object{
        object::pair{std:("skipPreflight"), true}, 
        object::pair{std:("commitment"), std:("confirmed")}
    });
    return std::make_shared<Program<std::shared_ptr<Autofun>>>(idl, provider);
};

void Main(void)
{
}

MAIN
