...
std::shared_ptr<Promise<string>> createSession(std::shared_ptr<SessionData> data)
{
    ...
    auto sid = uuid();
    std::async([=]() { redis->set(std:("sid:") + sid + string_empty, JSON->stringify(data), SESSION_TTL); });
    return sid;
};
...
{
    auto redis = std::async([=]() { getGlobalRedisCache(); });
    auto raw = std::async([=]() { redis->get(std:("sid:") + sid + string_empty); });
    return (raw) ? any((as<std::shared_ptr<SessionData>>(JSON->parse(raw)))) (nullptr);
};
...
{
    auto redis = std::async([=]() { getGlobalRedisCache(); });
    std::async([=]() { redis->del(std:("sid:") + sid + string_empty); });
};
