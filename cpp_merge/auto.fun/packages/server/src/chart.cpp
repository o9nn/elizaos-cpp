...
any getLatestCandle(string tokenMint, any swap, any tokenInfo)
{
    ...
    if (tokenInfo["status"] == std::string("locked")) {
        try
        {
            auto tokenAddress = (process->env->NETWORK == std::string("devnet")) ? any(DEV_TEST_TOKEN_ADDRESS) (tokenMint);
            auto candles = std::async([=]() { fetchCodexBars(tokenAddress, candleStart, candleStart + candlePeriod, std::string("1"), std::nullopt, std::nullopt); });
    ...
    if (tokenInfo["status"] != std::string("locked")) {
        ...
        )->map([=](auto swap) mutable
        {
            return (object{
                ...
                object::pair{std::string("priceUsd"), OR((swap["priceUsd"]), (std::nullopt))}, 
        ...
        try
        {
            ...
            auto candles = std::async([=]() { fetchCodexBars(tokenAddress, Math->floor(start / 1000), Math->floor(end / 1000), resolution, std::nullopt, std::nullopt); });
