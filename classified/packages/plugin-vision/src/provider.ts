    ...
    if (!cameraEnabled && !screenEnabled) {
      return {
        values: {
          ...
          screenStatus: screenEnabled
            ? 'Screen capture enabled but service disabled'
            : 'Screen capture disabled',
          microphoneStatus: microphoneEnabled
            ? 'Microphone enabled but service disabled'
            : 'Microphone disabled',
        },
        text: addHeader(
          '# Visual Perception',
          `Vision services are currently disabled. Camera: ${cameraEnabled ? 'enabled' : 'disabled'}, Screen: ${screenEnabled ? 'enabled' : 'disabled'}, Microphone: ${microphoneEnabled ? 'enabled' : 'disabled'}.`
        ),
