...
struct SummaryOptions {
    SummaryFormat format = SummaryFormat::CONDENSED;
 ...
 * 
 * High-level interface for Discord message summarization and analysis.
 * Automatically uses real or mock Discord client based on compilation flags.
 */
...
public:
    ...
    // Export operations
    bool exportSummaryToFile(const ChannelSummary& summary, const std::string& filename, const std::string& format = "json");
    bool exportMessagesToFile(const std::vector<DiscordMessage>& messages, const std::string& filename);
