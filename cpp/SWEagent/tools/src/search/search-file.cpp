...
void searchFile(const std::string& searchTerm, std::optional<std::string> filePath) {
    ...
    try {
        ...
        lines.forEach((line, index) => {
            ...
            // Print results
            std::cout << "Found " + matches.size() + " matches for \"" + searchTerm + "\" in " + filePath + ":" << std::endl;
            matches.forEach(match => {
                std::cout << "Line " + match.line + ":" + match.content << std::endl;
                });
                std::cout << "End of matches for \"" + searchTerm + "\" in " + filePath << std::endl;
