        ...
        },
        {
            ...
            },
            {
                ...
                for (const auto& pkg : corePackages)
                    std::cout << "Building " + pkg.name + "..." << std::endl;
                    const auto result = buildPackage(pkg.path, pkg.isPlugin);

                    if (!result.success) {
                        std::cerr << "Error building " + pkg.name + ": " + std::to_string(result.error.message) << std::endl;
                        } else {
                            std::cout << "✓ Built " + pkg.name + " (" + result.duration + "ms)" << std::endl;
                        }
...
std::future<void> performRebuild(DevContext context) {
    ...
    try {
        ...
        if (isMonorepo || directoryType.monorepoRoot) {
            ...
            if (result.success) {
                std::cout << "✓ Rebuild successful (" + result.duration + "ms)" << std::endl;
                } else {
