    ...
    if (localEmbeddingInfo) {
        ...
        if (envPreferences) {
            ...
            if (selectedProvider && Object.values(InferenceProvider).includes(selectedProvider)) {
                ...
                std::cout << "[INFERENCE] Using selected provider: " + selectedProvider << std::endl;
            }
                    ...
                    if (providerInfo.plugin.init) {
                        try {
                            ...
                            logger.debug(`[INFERENCE] Successfully initialized ${providerName}`);
                                ...
                                std::cout << "[INFERENCE] Failed to initialize " + providerName + ":" << initError << std::endl;
                        ...
                        std::cout << "[INFERENCE] Provider " + providerName + ": " + providerInfo.status << std::endl;
                        } catch (error) {
                            std::cerr << "[INFERENCE] Error checking provider " + providerName + ":" << error << std::endl;
                            providerInfo.status = "error";
...
std::future<bool> checkProviderValidity(IAgentRuntime runtime, InferenceProvider providerName, ProviderInfo providerInfo) {
    ...
    const auto validationServiceName = providerName + "-validation";
