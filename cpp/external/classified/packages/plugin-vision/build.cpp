...
std::future<void> build() {
    ...
    std::cout << "✅ Built " + mainResult.outputs.size() + " main files" << std::endl;
    ...
    try {
        ...
        .filter((f) => f.endsWith(".ts") && !(std::find(f.begin(), f.end(), "worker-logger") != f.end()));
        ...
        if (workerFiles.length > 0) {
            ...
            std::cout << "✅ Built " + workersResult.outputs.size() + " worker files" << std::endl;
            } else {
