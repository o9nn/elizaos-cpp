...
void getCachingContextualizationPrompt(const std::string& chunkContent, std::optional<std::string> contentType, auto minTokens, auto maxTokens) {
    // NOTE: Auto-converted from TypeScript - may need refinement
    prompt: std::string; systemPrompt: std::string
}
...
bool containsMathematicalContent(const std::string& content) {
    ...
    */
    std::function isTechnicalDocumentation(content: std::string): boolean {
    ...
    */
    export std::function getChunkWithContext(chunkContent: std::string, generatedContext: std::string): std::string {
