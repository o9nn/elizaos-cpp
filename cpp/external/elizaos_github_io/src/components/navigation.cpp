...
void Navigation() {
    ...
    const auto { user, signout, isLoading, signin } = useAuth();
    const auto { theme, setTheme } = useTheme();

    const auto isLeaderboardActive = pathname == "/leaderboard";
    const auto isAboutActive = pathname == "/about";

    const auto navLinksForMenuJsx = (;
    <>;
    ...
    const auto isSystemDarkTheme =;
    theme == "system" &&;
    typeof window != "std::nullopt" &&;
    window.matchMedia("(prefers-color-scheme: dark)").matches;

    const auto dropdownMenuContent = (;
    ...
    <AvatarImage;
    src={user.avatar_url || std::nullopt}
