...
public:
    ...
    std::tuple<std::string, string> call(P0 modelResponse, array<std::shared_ptr<Command>> _commands = undefined, boolean strict = undefined) = 0;
    template <typename P0>
    std::tuple<std::string, string> parse(P0 modelResponse, array<std::shared_ptr<Command>> commands = undefined, boolean strict = true);
};
...
public:
    ...
    std::string type = as<std::shared_ptr<const>>(std::string("thought_action"));

    template <typename P0>
    std::tuple<std::string, string> call(P0 modelResponse, array<std::shared_ptr<Command>> _commands = undefined, boolean strict = false);
};
...
public:
    ...
    std::string type = as<std::shared_ptr<const>>(std::string("action_only"));
...
public:
    ...
    std::string type = as<std::shared_ptr<const>>(std::string("xml_thought_action"));
...
public:
    ...
    std::string type = as<std::shared_ptr<const>>(std::string("function_calling"));

    virtual std::string formatErrorMessage(object error);
    template <typename P0>
    std::tuple<std::string, string> call(P0 modelResponse, array<std::shared_ptr<Command>> _commands = undefined, boolean strict = false);
};
...
public:
    ...
    std::string type = as<std::shared_ptr<const>>(std::string("single_bash_code_block"));
...
public:
    ...
    std::string type = as<std::shared_ptr<const>>(std::string("multiple_bash_code_blocks"));
...
public:
    ...
    std::string type = as<std::shared_ptr<const>>(std::string("identity"));

    template <typename P0>
    std::tuple<std::string, string> call(P0 modelResponse, array<std::shared_ptr<Command>> _commands = undefined, boolean _strict = false);
};
...
public:
    ...
    std::string type = as<std::shared_ptr<const>>(std::string("last_line"));
...
std::tuple<std::string, string> parseAction(P0 modelResponse, P1 parseFunction = std::string("thought_action"), array<std::shared_ptr<Command>> commands = undefined, boolean strict = false);
...
public:
    ...
    template <typename P0>
    std::tuple<std::string, string> call(P0 modelResponse, array<std::shared_ptr<Command>> _commands = undefined, boolean _strict = undefined);
};
...
public:
    ...
    std::string type = as<std::shared_ptr<const>>(std::string("json"));

    template <typename P0>
    std::tuple<std::string, string> call(P0 modelResponse, array<std::shared_ptr<Command>> _commands = undefined, boolean strict = undefined);
};

std::shared_ptr<AbstractParseFunction> createParser(std::string type);

template <typename P0>
std::tuple<std::string, string> AbstractParseFunction::parse(P0 modelResponse, array<std::shared_ptr<Command>> commands, boolean strict)
{
    return this->call(modelResponse, commands, strict);
}

template <typename P0>
std::tuple<std::string, string> ThoughtActionParser::call(P0 modelResponse, array<std::shared_ptr<Command>> _commands, boolean strict)
{
    ...
    if (codeBlockMatch) {
        ...
        return std::tuple<std::string, string>{ thought, action };
    }
    ...
    auto thought = (thoughtMatch) ? std::any((*const_(thoughtMatch))[1]->trim()) : std::any(string_empty);
    auto action = (actionMatch) ? std::any((*const_(actionMatch))[1]->trim()) : std::any(string_empty);
...
"));
        for (auto i = 0; i < lines->get_length(); i++)
        {
            ...
            if (line->toLowerCase()->includes(std::string("thought:"))) {
                ...
                return std::tuple<std::string, string>{ thoughtLines->join(std::string("\
"))->trim(), string_empty };
            }
            if (line->toLowerCase()->includes(std::string("action:"))) {
                ...
                return std::tuple<std::string, string>{ OR((thought), (string_empty)), actionLines->join(std::string("\
"))->trim() };
            }
        }
    }
    if (AND((strict), (!action))) {
        throw std::any(std::make_shared<FormatError>(std::string("Could not parse action from model response")));
    }
    return std::tuple<std::string, string>{ thought, action };
}

template <typename P0>
std::tuple<std::string, string> ActionOnlyParser::call(P0 modelResponse, array<std::shared_ptr<Command>> _commands, boolean strict)
{
    auto message = (type_of(modelResponse) == std::string("string")) ? modelResponse : OR((OR((modelResponse->message), (modelResponse->content))), (string_empty));
    ...
    if (AND((AND((strict), (_commands))), (_commands->get_length() > 0))) {
        ...
        if (!validCommand) {
            throw std::any(std::make_shared<FormatError>(std::string("Invalid command: ") + commandName + string_empty));
        }
    }
    return std::tuple<std::string, string>{ string_empty, action };
}

template <typename P0>
std::tuple<std::string, string> XMLThoughtActionParser::call(P0 modelResponse, array<std::shared_ptr<Command>> _commands, boolean strict)
{
    auto message = (type_of(modelResponse) == std::string("string")) ? modelResponse : OR((OR((modelResponse->message), (modelResponse->content))), (string_empty));
    ...
    auto thought = (thoughtMatch) ? std::any((*const_(thoughtMatch))[1]->trim()) : std::any(string_empty);
    auto action = (actionMatch) ? (*const_(actionMatch))[1]->trim() : (commandMatch) ? std::any((*const_(commandMatch))[1]->trim()) : std::any(string_empty);
    if (AND((commandMatch), (!thought))) {
        ...
        return std::tuple<std::string, string>{ thoughtText, (*const_(commandMatch))[1]->trim() };
    }
    if (AND((strict), (!action))) {
        throw std::any(std::make_shared<FormatError>(std::string("Could not parse action from model response")));
    }
...
template <typename P0>
std::tuple<std::string, string> EditFormatParser::call(P0 modelResponse, array<std::shared_ptr<Command>> _commands, boolean strict)
{
    ...
    if (OR((action->includes(std::string("str_replace_editor"))), (action->includes(std::string("str_replace_based_edit_tool"))))) {
        ...
        if (editMatch) {
            return std::tuple<std::string, string>{ thought, (*const_(editMatch))[1]->trim() };
        }
    }
    return std::tuple<std::string, string>{ thought, action };
}
...
template <typename P0>
std::tuple<std::string, string> FunctionCallingParser::call(P0 modelResponse, array<std::shared_ptr<Command>> _commands, boolean strict)
{
    auto toolCalls = (type_of(modelResponse) != std::string("string")) ? std::any(OR((modelResponse->toolCalls), ((as<any>(modelResponse))["tool_calls"]))) : std::any(nullptr);
    if (AND((toolCalls), (toolCalls["length"] > 0))) {
        if (AND((toolCalls["length"] > 1), (strict))) {
            throw std::any(std::make_shared<FormatError>(std::string("Multiple tool calls not supported")));
        }
        ...
        if (AND((AND((_commands), (_commands->get_length() > 0))), (strict))) {
            ...
            if (!validCommand) {
                throw std::any(std::make_shared<FormatError>(std::string("Invalid command: ") + functionName + string_empty));
            }
        }
        ...
        if (args) {
                ...
                if (AND((AND((argsObj), (type_of(argsObj) == std::string("object")))), (Object->keys(argsObj)->get_length() > 0))) {
                    if (argsObj["command"]) {
                        ...
                        if (values->get_length() > 0) {
                            action = string_empty + functionName + std::string(" ") + values->std::map([=](auto v) mutable
                            {
                                return String(v);
                            }
            ...
            {
                if (strict) {
                    throw std::any(std::make_shared<FormatError>(std::string("Invalid JSON in std::function arguments")));
                }
            }
        }
        auto thought = (type_of(modelResponse) != std::string("string")) ? std::any(OR((modelResponse->message), (string_empty))) : std::any(string_empty);
        return std::tuple<std::string, any>{ thought, action };
    }
    if (strict) {
        throw std::any(std::make_shared<FormatError>(std::string("No tool calls found in model response")));
    }
    auto message = (type_of(modelResponse) == std::string("string")) ? modelResponse : OR((modelResponse->message), (string_empty));
    return std::tuple<std::string, string>{ string_empty, message };
}

template <typename P0>
std::tuple<std::string, string> SingleBashCodeBlockParser::call(P0 modelResponse, array<std::shared_ptr<Command>> _commands, boolean strict)
{
    auto message = (type_of(modelResponse) == std::string("string")) ? modelResponse : OR((OR((modelResponse->message), (modelResponse->content))), (string_empty));
    auto codeBlockMatch = message->match((new RegExp(std::string("```(?:bash|sh)?\n(.*?)```"))));
    if (codeBlockMatch) {
        return std::tuple<std::string, string>{ string_empty, (*const_(codeBlockMatch))[1]->trim() };
    }
    if (strict) {
        throw std::any(std::make_shared<FormatError>(std::string("Could not find bash code block in model response")));
    }
    return std::tuple<std::string, string>{ string_empty, message->trim() };
}

template <typename P0>
std::tuple<std::string, string> MultipleBashCodeBlocksParser::call(P0 modelResponse, array<std::shared_ptr<Command>> _commands, boolean strict)
{
    auto message = (type_of(modelResponse) == std::string("string")) ? modelResponse : OR((OR((modelResponse->message), (modelResponse->content))), (string_empty));
    ...
    if (codeBlocks->get_length() > 0) {
        return std::tuple<std::string, string>{ string_empty, codeBlocks->join(std::string("\
\
")) };
    }
    if (strict) {
        throw std::any(std::make_shared<FormatError>(std::string("Could not find bash code blocks in model response")));
    }
    return std::tuple<std::string, string>{ string_empty, message->trim() };
}

template <typename P0>
std::tuple<std::string, string> Identity::call(P0 modelResponse, array<std::shared_ptr<Command>> _commands, boolean _strict)
{
    auto message = (type_of(modelResponse) == std::string("string")) ? modelResponse : OR((OR((modelResponse->message), (modelResponse->content))), (string_empty));
    auto codeBlockMatch = message->match((new RegExp(std::string("```(?:bash|sh)?\n(.*?)```"))));
    if (codeBlockMatch) {
        auto action = (*const_(codeBlockMatch))[1]->trim();
        return std::tuple<std::string, string>{ message, action };
    }
    return std::tuple<std::string, string>{ message, message };
}

template <typename P0>
std::tuple<std::string, string> LastLineParser::call(P0 modelResponse, array<std::shared_ptr<Command>> _commands, boolean strict)
{
    auto message = (type_of(modelResponse) == std::string("string")) ? modelResponse : OR((OR((modelResponse->message), (modelResponse->content))), (string_empty));
...
"))->trim();
    return std::tuple<std::string, string>{ thought, lastLine };
}

template <typename P0, typename P1>
std::tuple<std::string, string> parseAction(P0 modelResponse, P1 parseFunction, array<std::shared_ptr<Command>> commands, boolean strict)
{
...
template <typename P0>
std::tuple<std::string, string> IdentityParser::call(P0 modelResponse, array<std::shared_ptr<Command>> _commands, boolean _strict)
{
    auto content = (type_of(modelResponse) == std::string("object")) ? OR((modelResponse->message), (string_empty)) : modelResponse;
    return std::tuple<std::string, string>{ string_empty, content->trim() };
}

template <typename P0>
std::tuple<std::string, string> JsonParser::call(P0 modelResponse, array<std::shared_ptr<Command>> _commands, boolean strict)
{
    auto message = (type_of(modelResponse) == std::string("string")) ? modelResponse : OR((OR((modelResponse->message), (modelResponse->content))), (string_empty));
    try
    {
        ...
        if (OR((!parsed["thought"]), (!parsed["command"]))) {
            if (strict) {
                throw std::any(std::make_shared<FormatError>(std::string("JSON must contain both "thought" and "command" keys")));
            }
            return std::tuple<std::string, string>{ string_empty, message };
        }
        ...
        return std::tuple<std::any, string>{ thought, action };
    }
    catch (const std::any& e)
    {
        if (strict) {
            throw std::any(std::make_shared<FormatError>(std::string("Invalid JSON format")));
        }
        return std::tuple<std::string, string>{ string_empty, message };
    }
}
