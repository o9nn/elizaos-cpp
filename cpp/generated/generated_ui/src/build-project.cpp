...
void buildProject(std::string cwd, boolean isPlugin)
{
    ...
    if (!fs->existsSync(cwd)) {
        throw std::any(std::make_shared<Error>(std::string("Project directory ") + cwd + std::string(" does not exist.")));
    }
    ...
    if (!dirInfo->hasPackageJson) {
        ...
        throw std::any(std::make_shared<Error>(std::string("Project directory ") + cwd + std::string(" does not have package.json.")));
    }
    ...
    try
    {
        ...
        if (packageJson["scripts"]["build"]) {
            ...
            catch (const std::any& buildError)
            {
                logger->debug(std::string("Bun build failed: ") + buildError + string_empty);
                throw std::any(std::make_shared<Error>(std::string("Failed to build using bun: ") + buildError + string_empty));
            }
        }
        ...
        if (fs->existsSync(tsconfigPath)) {
            ...
            catch (const std::any& tscError)
            {
                logger->debug(std::string("bunx tsc build failed: ") + tscError + string_empty);
            }
        }
        throw std::any(std::make_shared<Error>(std::string("Could not determine how to build the project")));
    }
