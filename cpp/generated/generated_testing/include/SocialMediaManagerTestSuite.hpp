...
public:
    ...
    std::string name = std::string("social-media-manager");

    std::string description = std::string("Tests for the social media manager agent");
    ...
    array<object> tests = array<object>{ object{
        ...
        object::pair{std::string("fn"), [=](auto runtime) mutable
        {
            ...
            std::async([=]() { this->scenarioService["sendMessage"](runtime, worldId, mainRoomId, std::string("Hi! I'd like to std::set up social media management for my organization.")); });
            auto completed = std::async([=]() { this->scenarioService["waitForCompletion"](this->completionTimeout); });
            if (!completed) {
                throw std::any(std::make_shared<Error>(std::string("Agent did not complete onboarding response in time")));
            }
        }
        }
    }, object{
        ...
        object::pair{std::string("fn"), [=](auto runtime) mutable
        {
            ...
            auto completed = std::async([=]() { this->scenarioService["waitForCompletion"](this->completionTimeout); });
            if (!completed) {
                throw std::any(std::make_shared<Error>(std::string("Agent did not complete post creation in time")));
            }
        }
        }
    }, object{
        ...
        object::pair{std::string("fn"), [=](auto runtime) mutable
        {
            ...
            for (auto& query : queries)
            {
                ...
                auto completed = std::async([=]() { this->scenarioService["waitForCompletion"](this->completionTimeout); });
                if (!completed) {
                    throw std::any(std::make_shared<Error>(std::string("Agent did not respond to query in time: ") + query + string_empty));
                }
            }
        }
        }
