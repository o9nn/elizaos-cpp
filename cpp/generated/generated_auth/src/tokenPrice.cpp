#include "trust_scoreboard/src/pages/api/tokenPrice.h"
...
std::any handler(std::shared_ptr<NextApiRequest> req, std::shared_ptr<NextApiResponse> res)
{
    ...
    {
        if (!HELIUS_API) {
            throw std::any(std::make_shared<Error>(std::string("Helius API key not configured")));
        }
        ...
        if (!response->ok) {
            throw std::any(std::make_shared<Error>(std::string("Helius API error: ") + response->status + string_empty));
        }
    ...
    catch (const std::any& error)
    {
        ...
        res->status(200)->json(object{
            object::pair{std::string("prices"), TOKEN_ADDRESSES->std::map([=](auto address) mutable
            {
                return (object{
...
std::string HELIUS_API = process->env->NEXT_PUBLIC_SOLANA_API;
