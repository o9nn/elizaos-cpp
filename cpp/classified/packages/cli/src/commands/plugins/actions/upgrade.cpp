                ...
                if (existsSync(monorepoGuidesPath)) {
                    guidesSource = monorepoGuidesPath;
                        ...
                        if (guidesSource && existsSync(guidesSource)) {
                            ...
                            if (opts.debug) {
                                std::cout << "chalk.gray(" + "Copied guides from: " + guidesSource << std::endl;
                            }
                            } else {
                            ...
                            if (opts.debug) {
                                std::cout << "chalk.gray(" + "\nPlugin: " + pluginPath << std::endl;
                                std::cout << "chalk.gray(" + "Working directory: " + workingDir << std::endl;
                            }
                                ...
                                agent.on("progress", (count) => {
                                    ...
                                    try {
                                        if (existsSync(guidesTarget)) {
                                            ...
                                            if (opts.debug) {
                                                std::cout << "chalk.yellow(" + "Warning: Could not clean up guides: " + cleanupError << std::endl;
                                            }
                                        }
                                        ...
                                        if (result.success) {
                                            ...
                                            if (result.guidesUsed && result.guidesUsed.length > 0) {
                                                std::cout << "Migration Guides Used: " + std::to_string(chalk.magenta(result.guidesUsed.size())) << std::endl;
                                                if (opts.debug) {
                                            ...
                                            std::cout << std::to_string(chalk.gray("6.")) + " Merge to main and publish\n" << std::endl;
                                                ...
                                                if (result.error) {
                                                    ...
                                                    std::cout << "chalk.red(" + "💥 " + errorMsg << std::endl;
