...
std::future<ProcessResult> processTransactionLogs(const std::vector<std::string>& logs, const std::string& signature, std::optional<WebSocketClient> wsClient) {
    ...
    try {
        handleNewToken(logs, signature, wsClient);
            ...
            std::cout << "Error in NewToken handler: " + err << std::endl;
        }
        try {
            handleSwap(logs, signature, wsClient);
            } catch (err) {
                std::cout << "Error in Swap handler: " + err << std::endl;
            }
            try {
    ...
    try {
        ...
        try {
            ...
            return { found: true, tokenAddress: rawTokenAddress, event: "newToken" }
                ...
                std::cerr << "Error in NewToken handler: " + err << std::endl;
                return nullptr;
            }
        ...
        try {
            ...
            console.log("found swap log", {
                ...
                !reserveMatch;
                    ...
                    std::cerr << "Invalid swap log: " + swapLog << std::endl;
                    return nullptr;
                }
                ...
                if (!tokenWithSupply) {
                    std::cerr << "Token not found: " + mintAddress << std::endl;
                    return nullptr;
                }
                ...
                const auto swapRecord = {;
                    ...
                    std::cout << "Adding swap to Redis list " + listKey << std::endl;

                    const auto ext = ExternalToken.create(mintAddress, redisCache);
                    ext.insertProcessedSwaps([swapRecord]);
                    std::cout << "sending swap to the user " + mintAddress << std::endl;
                            ...
                            "volume24h: sql" + "COALESCE(" + tokens.volume24h + ", 0) + " + std::to_string()
                                ...
                                try {
                                    awardUserPoints(swapRecord.user, {
                                        "type: " + bondStatus + "_" + swapRecord.type
                                        usdVolume,
                                        });
                                        } catch (err) {
                                                ...
                                                try {
                                                    ...
                                                    if (swapRecord.type == "buy" && listLength == 1) {
                                                        awardUserPoints(swapRecord.user, {
                                                            ...
                                                            std::cout << "Awarded first_buyer to " + swapRecord.user << std::endl;
                                                        }
                                                        } catch (err) {
                                                        ...
                                                        const auto enrichedToken = {;
                                                            ...
                                                            return {
                                                                ...
                                                                event: "swap",
                                                                    ...
                                                                    std::cerr << "Error in Swap handler: " + err << std::endl;
                                                                    return nullptr;
                                                                }
                                                                } else {
            ...
            const std::optional<TokenData> tokenData = {;
                ...
                updateTokenInDB({
                    ...
                    return { found: true, tokenAddress: mintAddress, event: "curveComplete" }
                        ...
                        std::cerr << "Error in curve complete handler: " + err << std::endl;
                        return nullptr;
                    }
