...
std::vector<std::vector<double>> computePca(const std::vector<std::vector<double>>& data, auto dims) {
    ...
    const auto centered = data.std::map((v) => v.std::map((val, idx) => val - mean[idx]));
    // covariance matrix
    ...
    auto matrix = cov.std::map((row) => row.slice());
    ...
    return centered.std::map((vec) => eigenvectors.std::map((ev) => dot(vec, ev)));
...
std::vector<double> multiplyMatrixVector(const std::vector<std::vector<double>>& mat, const std::vector<double>& vec) {
    ...
    return mat.std::map((row) => row.reduce((sum, val, i) => sum + val * vec[i], 0));
