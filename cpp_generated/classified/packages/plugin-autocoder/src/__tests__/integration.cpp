...
std::function<std::shared_ptr<Promise<any>>()> createRealRuntime = [=]() mutable
{
    ...
    elizaLogger->info(std::string("Runtime initialized"), object{
        object::pair{std::string("hasForms"), runtime->getService(std::string("forms")) != undefined}, 
        object::pair{std::string("hasCodeGen"), runtime->getService(std::string("code-generation")) != undefined}
    });
    return runtime;
};
