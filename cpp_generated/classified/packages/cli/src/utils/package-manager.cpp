#include "/home/runner/work/elizaos-cpp/elizaos-cpp/classified/packages/cli/src/utils/package-manager.h"
...
std::shared_ptr<Promise<void>> removeFromBunLock(string packageName, string directory)
{
    ...
    try
    {
        auto result = std::async([=]() { bunExec(std::string("bun"), array<string>{ std::string("remove"), packageName }, object{
            object::pair{std::string("cwd"), directory}
        }); });
        if (result->success) {
            logger->debug(std::string("Successfully removed ") + packageName + std::string(" from bun.lock"));
        } else {
            if (OR((result->stderr->includes(std::string("not found"))), (result->stderr->includes(std::string("No such package"))))) {
                logger->debug(std::string("Package ") + packageName + std::string(" not found in lockfile (expected for cleanup)"));
            } else {
                logger->warn(std::string("Failed to remove ") + packageName + std::string(" from bun.lock: ") + (OR((result->stderr), (std::string("Unknown error")))) + string_empty);
            }
        }
    }
    catch (const any& error)
    {
        logger->warn(std::string("Unexpected error removing ") + packageName + std::string(" from bun.lock: ") + error["message"] + string_empty);
    }
};
