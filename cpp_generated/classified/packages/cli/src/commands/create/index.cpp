...
{
    return (type == std::string("tee")) ? any(std::string("TEE Project")) : any(type->charAt(0)->toUpperCase() + type->slice(1));
};


any create = ((std::make_shared<Command>(std::string("create"))))->description(std::string("Create a new ElizaOS project, plugin, agent, or TEE project"))->argument(std::string("[name]"), std::string("name of the project/plugin/agent to create"))->option(std::string("--yes, -y"), std::string("skip prompts and use defaults"))->option(std::string("--type <type>"), std::string("type of project to create (project, plugin, agent, tee)"), std::string("project"))->action([=](auto name = undefined, auto opts = undefined) mutable
{
