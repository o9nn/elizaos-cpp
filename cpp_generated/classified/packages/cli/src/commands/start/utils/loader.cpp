#include "/home/runner/work/elizaos-cpp/elizaos-cpp/classified/packages/cli/src/commands/start/utils/loader.h"

any tryLoadFile(string filePath)
{
    auto serverModule = loadModuleSync(std::string("@elizaos/server"));
...
{
    auto serverModule = std::async([=]() { loadModule(std::string("@elizaos/server")); });
    return serverModule->tryLoadFile(filePath);
};
