#include "/home/runner/work/elizaos-cpp/elizaos-cpp/classified/packages/cli/tests/commands/publish.test.h"
...
exit /b 0")); });
                ...
                bunExecSync(std::string("chmod +x ") + join(mockBinDir, std::string("npm")) + string_empty);
            }
...
case "$1" in\
  ...
  *)\
    ...
    ;\
            ...
            if (process->platform != std::string("win32")) {
                bunExecSync(std::string("chmod +x ") + join(mockBinDir, std::string("git")) + string_empty);
            }
...
case "$1" in\
  ...
  *)\
    ...
    ;\
            ...
            if (process->platform != std::string("win32")) {
                bunExecSync(std::string("chmod +x ") + join(mockBinDir, std::string("gh")) + string_empty);
            }
        }
        );
        afterEach([=]() mutable
        ...
        {
            auto result = bunExecSync(std::string("elizaos publish --help"), object{
        ...
        {
            auto helpResult = bunExecSync(std::string("elizaos --help"), object{
                object::pair{std::string("encoding"), std::string("utf8")}
            });
            expect(helpResult)->toContain(std::string("publish"));
            auto publishHelpResult = bunExecSync(std::string("elizaos publish --help"), object{
                object::pair{std::string("encoding"), std::string("utf8")}
            });
        ...
        {
            auto result = bunExecSync(std::string("elizaos publish --dry-run --help"), object{
                object::pair{std::string("encoding"), std::string("utf8")}
            });
            expect(result)->toContain(std::string("dry-run"));
        }
