...
std::shared_ptr<Promise<void>> waitForServerReady(double port, double maxWaitTime, string endpoint)
{
    ...
    shared pollInterval = (AND((isMacOS), (isCI))) ? any(3000) : any((isMacOS) ? 2000 : 1000);
    shared requestTimeout = (AND((isMacOS), (isCI))) ? any(6000) : any((isMacOS) ? 4000 : 2000);
    ...
    while (Date->now() - startTime < maxWaitTime)
    {
        try
        {
            ...
            if (response->ok) {
                ...
                shared stabilizationTime = (AND((isMacOS), (isCI))) ? any(3000) : any((isMacOS) ? 2000 : 1000);
        ...
        {
            auto timeRemaining = maxWaitTime - (Date->now() - startTime);
            auto errorMsg = (is<Error>(error)) ? any(error->message) : any(std::string("unknown error"));
...
"))->filter([=](auto pid) mutable
    ...
    {
        console->log(std::string("[DEBUG] Port cleanup for ") + port + std::string(" encountered error:"), (is<Error>(e)) ? any(e->message) : any(std::string("unknown")));
    }
    return std::shared_ptr<Promise<void>>();
};
