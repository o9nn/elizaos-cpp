#include "/home/runner/work/elizaos-cpp/elizaos-cpp/classified/packages/agentserver/src/api/runtime/logging.h"
        ...
        try
        {
            ...
            auto filtered = recentLogs->filter([=](auto log) mutable
            {
                ...
                if (AND((requestedLevel), (requestedLevel != std::string("all")))) {
                    auto logLevel = (type_of(log->level) == std::string("number")) ? any(log->level) : any(OR((const_(LOG_LEVELS)[as<any>(log->level)]), (30)));
