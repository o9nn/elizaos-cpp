...
"), responsePrompt);
        ...
        if (safeXml["simple"]) {
            ...
            )->join(std::string(", "))) : any(safeXml["actions"])) + std::string(" | ") + ((type_of(safeXml["providers"]) == std::string("string")) ? any(safeXml["providers"]->split(std::string(","))->map([=](auto provider) mutable
            {
                return provider->trim();
            }
            )->join(std::string(", "))) : any(safeXml["providers"])));
