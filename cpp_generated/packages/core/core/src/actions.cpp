#include "/home/runner/work/elizaos-cpp/elizaos-cpp/eliza/packages/core/src/actions.h"
...
string formatActions(array<std::shared_ptr<Action>> actions)
{
    ...
    {
        return string_empty + action->name + std::string(": ") + action->description + string_empty;
    }
    )->join(std::string(",\
"));
};
