#include "/home/runner/work/elizaos-cpp/elizaos-cpp/otaku/src/packages/server/src/api/agents/lifecycle.h"
...
std::shared_ptr<express::Router> createAgentLifecycleRouter(std::shared_ptr<ElizaOS> elizaOS, std::shared_ptr<AgentServer> serverInstance)
{
    ...
    router->post(std::string("/:agentId/start"), requireAuth, requireAdmin, [=](auto req, auto res) mutable
    {
        ...
        {
            logger->error(std::string("[AGENT START] Error starting agent:"), (is<Error>(error)) ? error->message : String(error));
            sendError(res, 500, std::string("START_ERROR"), std::string("Error starting agent"), (is<Error>(error)) ? error->message : String(error));
        }
    }
    );
    router->post(std::string("/:agentId/stop"), requireAuth, requireAdmin, [=](auto req, auto res) mutable
    {
        auto agentId = validateUuid(req->params->agentId);
        if (!agentId) {
