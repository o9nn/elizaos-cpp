#include "/home/runner/work/elizaos-cpp/elizaos-cpp/otaku/src/plugins/plugin-bootstrap/src/providers/world.h"
...
std::shared_ptr<Provider> worldProvider = object{
    ...
    object::pair{std::string("get"), [=](auto runtime, auto message) mutable
    {
        try
        {
            ...
            }, std::string("[] World provider activated for roomId:"));
            ...
            if (!worldId) {
                logger->warn(std::string("World provider: World ID not found for roomId ") + message->roomId + string_empty);
                return object{
                    object::pair{std::string("data"), object{
                        object::pair{std::string("world"), object{
                            object::pair{std::string("info"), std::string("Unable to retrieve world information - world ID not found")}
                        }}
                    }}, 
                    object::pair{std::string("text"), std::string("Unable to retrieve world information - world ID not found")}
                };
            }
            ...
            for (auto& room : worldRooms)
            {
                if (OR((!room->id), (!room->name))) {
                    logger->warn(std::string("World provider: Room ID or name is missing for room ") + room->id + string_empty);
                    continue;
                }
